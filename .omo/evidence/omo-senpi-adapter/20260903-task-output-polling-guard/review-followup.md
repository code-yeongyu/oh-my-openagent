...
The current-head review correctly identified two remaining gaps. The scoped driver now proves the actual generated bundle's second unchanged `status` read returns `no_progress`, rather than only exercising `mode:"tail"`. The terminal guidance now uses `record.notify_on_terminal`, not a hand-maintained list of terminal statuses, so foreground, completed, error, lost, cancelled, and interrupted records are all directed according to whether a notification can actually arrive.
