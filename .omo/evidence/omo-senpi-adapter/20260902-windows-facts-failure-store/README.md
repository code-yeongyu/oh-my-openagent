...
The repair schedules each directory's advisory sweep once per candidate stale-age window. The immediate next acquisition skips a fresh sharing-blocked candidate. Once eligible, one stale sweep runs; if sharing still blocks it, the directory is not eligible again until the next age window. A non-sharing candidate cleanup error, including one observed by the stale sweep itself, deletes that directory's eligibility entry so the next acquisition may sweep again. Candidates never establish ownership after hard-link publication, so this changes only advisory cleanup, not mutual exclusion.
...
- `red-nonsharing-stale-sweep.log` and `green-nonsharing-stale-sweep.log` prove the stale-sweep `EIO` rearm fix (RED then 8 pass). Final source commit `09995e71ff243256b9f88ba8b1f15cb85f70073e` passed `final-focused.log` (17 pass), `final-memory-core.log` (654 pass), `final-typecheck.log`, `final-generated-bundle-check.log`, and `final-test-senpi.log` (2,530 pass, 1 Windows-only skip, 0 fail). `final-senpi-drive-live.json` records a distinct real-driver PASS on that exact commit with sandbox and child-process absence.
...
`cleanup-receipt.txt`, `post-merge-driver-cleanup-results.txt`, and `final-driver-cleanup-results.txt` record sandbox and child-process absence separately for each self-test and live-driver capture. `post-merge-sandbox-process-investigation.txt` separately documents an earlier driver-associated TypeScript installer that was still alive after the driver returned; its process tree was manually terminated and verified absent. No driver-owned cleanup is claimed for that incident. No OpenCode or Codex production source changed; no real OpenCode or Codex harness was required. The only shipped adapter changes are regenerated Senpi bundles, covered by the Senpi gate and live isolated QA above.
