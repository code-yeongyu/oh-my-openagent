...
EVIDENCE="$ROOT/.omo/evidence/20260718-pr-6043-abort-owner-overlap-repair"
BASE_SCRIPT="$ROOT/.omo/evidence/20260718-pr-6043-overlap-ownership-repair/run-live-watchdog-qa.sh"
TMP_SCRIPT="$(mktemp -t pr6043-owner-live.XXXXXX)"
SOURCE_PATHS=(
  packages/omo-opencode/src/hooks/runtime-fallback/message-update-handler.ts
  packages/omo-opencode/src/hooks/runtime-fallback/retry-owner-abort-overlap.test.ts
  packages/omo-opencode/src/hooks/runtime-fallback/session-retry-ownership.ts
  packages/omo-opencode/src/hooks/runtime-fallback/session-status-handler.ts
)
...
sed \
  -e "s#EVIDENCE=\"/tmp/pr6043-overlap-live-evidence\"#EVIDENCE=\"$EVIDENCE\"#" \
  -e 's#test -z "$(git status --porcelain=v1)"#:#' \
  "$BASE_SCRIPT" > "$TMP_SCRIPT"
chmod +x "$TMP_SCRIPT"

printf 'base_head=%s\nsource_diff_sha256=%s\nsource_files_sha256_before=%s\n' \
  "$BASE_HEAD" "$SOURCE_DIFF_SHA256" "$SOURCE_FILES_SHA256_BEFORE" \
  > "$EVIDENCE/live-working-tree-source.txt"
