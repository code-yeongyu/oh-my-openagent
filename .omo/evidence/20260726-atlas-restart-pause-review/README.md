...
- Ran package typechecks for `packages/boulder-state` and `packages/omo-opencode`, LSP diagnostics for both changed TypeScript files, a restricted-pattern audit, and a direct bundle of the OpenCode plugin entry.
...
- Both affected package typechecks exited 0. LSP reported no diagnostics. The restricted-pattern audit found no forbidden constructs.
