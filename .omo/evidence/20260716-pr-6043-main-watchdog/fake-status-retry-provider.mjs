...
function sendChatText(response, id, model, text) {
...
function sendResponsesText(response, id, model, text) {
  response.writeHead(200, {
    "content-type": "text/event-stream",
    "cache-control": "no-cache",
    connection: "keep-alive",
  ...
  const events = [
    { type: "response.created", response: { id, object: "response", status: "in_progress", model, output: [] } },
    ...
    { type: "response.completed", response: { id, object: "response", status: "completed", model, output: [{ id: `${id}-msg`, type: "message", role: "assistant", status: "completed", content: [{ type: "output_text", text, annotations: [] }] }], usage: { input_tokens: 1, output_tokens: 1, total_tokens: 2 } } },
  ]
...
const server = http.createServer(async (request, response) => {
  ...
  if (model === "primary") {
    response.writeHead(429, {
      ...
      error: {
        message: "Rate limit reached; retry after 120 seconds",
        type: "rate_limit_error",
        code: "rate_limit_exceeded",
      },
    }))
    log("PRIMARY_429_SENT")
    return
  }

  if (model === "fallback-one") {
    const id = `status-${Date.now()}`
    if (isChatCompletions) sendChatText(response, id, model, "QA_STATUS_FALLBACK_OK")
    else sendResponsesText(response, id, model, "QA_STATUS_FALLBACK_OK")
    log("FALLBACK_ONE_RESPONSE_SENT")
    return
  }

  if (model === "fallback-two") {
    const id = `stale-${Date.now()}`
    if (isChatCompletions) sendChatText(response, id, model, "UNEXPECTED_STALE_WATCHDOG_FALLBACK")
    else sendResponsesText(response, id, model, "UNEXPECTED_STALE_WATCHDOG_FALLBACK")
    log("FALLBACK_TWO_RESPONSE_SENT")
    return
  }
