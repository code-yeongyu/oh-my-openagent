...
**`ulw-plan` consults on the finished draft instead of the empty skeleton.** ([#8509](https://github.com/code-yeongyu/oh-my-openagent/issues/8509)) The plan consultant used to be spawned right after the plan skeleton was created, before a single todo, acceptance criterion or QA scenario existed, so it could only comment on the request. Every edition (OmO Native, OpenCode, Codex) now writes the complete initial draft first (all todos, the human summary, the structural self-check), hands the consultant the exact `.omo/plans/<slug>.md` path, folds its findings, repeats the self-check when the plan changed, and only then starts the high-accuracy review. An owner decision the consultant surfaces returns to the approval gate instead of being defaulted. The OmO Native and OpenCode consultant agents gained a plan-draft audit mode that checks todo rows, dependency matrices, acceptance criteria and QA scenarios when given a plan path; the Codex analyst already did. The `/ulw-execute` bootstrap path is unchanged.
