...
- Root-cause note [2026-04-01]: commit `b3295fce` regressed internal prompt transport by replacing invisible HTML-style wrappers (`<system-reminder>`, `<system-directive>`, `<!-- OMO_INTERNAL_INITIATOR -->`) with plain markdown separators plus visible `[OMO_INTERNAL]`. Result: service-only injections started appearing in persisted chat history, and models could echo `[SYSTEM DIRECTIVE: OH-MY-OPENCODE - ...]` blocks verbatim into visible replies.
- Delegation root-cause note [2026-04-01]: a subagent verdict session can show a confident `PASS` with zero tool calls because `task()` currently accepts evidence-free review prompts like `EXPECTED OUTCOME: PASS or FAIL verdict` + `REQUIRED TOOLS: none`. In that shape the child agent is not mis-rendered and the UI is not hiding anything; the prompt itself authorized conclusion-by-reasoning. Structural fix belongs in `src/tools/delegate-task/`: review/audit/verify/confirm/verdict prompts must either name an evidence path (explicit tools, session_id continuation, file/log/DB target, or prior result source) or be rejected before launch.
- Root-cause note [2026-04-01]: `BackgroundManager.injectPendingNotificationsIntoChatMessage()` prepends queued background notifications directly into the next user text part. If the queued notification is rendered as plain text instead of an invisible wrapper, it contaminates the actual user prompt and compounds with keyword-detector injections like `[search-mode]` / `[analyze-mode]`, producing giant synthetic prompts in parent sessions.
- User-correction note [2026-04-01]: leaked internal prompts must not be “fixed” by UI-side hiding. OMO must mark internal prompt parts as structurally non-user-visible (for example `synthetic: true`) at creation time, so normal chat rendering stays faithful without frontend sanitizing. If a fix relies on UI filtering, it is the wrong layer.
- Implementation note [2026-04-01]: OpenCode `promptAsync` and stored text parts natively support `synthetic: true`, and the UI/TUI already exclude synthetic text from normal transcript rendering. Internal OMO prompt injections (`createInternalAgentTextPart`, system reminders/directives, queued pending notifications) should use synthetic parts instead of visible text wrappers whenever the content is orchestration-only.
