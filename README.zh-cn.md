...
|   🧩   | **Codex CLI Light Edition**                                      | Light    | 在 OpenAI Codex CLI 中运行的 omo 的 5 个可移植组件 (rules, comment-checker, LSP, ultrawork, ulw-loop)。安装: `bunx --package oh-my-openagent omo install --platform=codex`。                              |
...
> **Editions 图例。** **Ultimate** = 仅 OpenCode (`bunx --package oh-my-openagent omo install`)。**Light** = 仅 Codex CLI (`bunx --package oh-my-openagent omo install --platform=codex`)。**Both** = 两个版本均提供 (内部实现可能略有不同)。
