...
- **2026-07-14 — CONSOLIDATION Magic Context + rebranding DÉMARRÉE** : les Waves 1–5 MC sont **codées en legacy** (`MaTrix-test` main, commits `ecdf85b96` + `0c7e3a1d9`, source `engine/packages/omo-opencode/src/features/magic-context/`). Consolidation vers le fork propre **`matrix-port`** (base `upstream/dev`) EN COURS — port additif + re-câblage manuel de `create-transform-hooks.ts` / `messages-transform.ts` / `tool-registry.ts` (la base upstream diffère du legacy). Build + typecheck + tests obligatoires ; **PAS de self-test `sse-hook-probe.sh --self-test`** (serveur non-terminant → a crashé opencode le 2026-07-14). Rebranding (Morpheus/Neo/Tank/Operator/Ghost/Link/Analyst/Keymaker/AgentSmith) localisé dans `engine/.../agents/builtin-agents.ts` + `morpheus*`.
  ...
  - **2026-07-14 — CONSOLIDATION Magic Context TERMINÉE** : port additif (40 fichiers `features/magic-context/` + `config/schema/magic-context.ts` + wiring 4 composers) → commit `47d31f557` branche `matrix-magic-context` → **PR #6126**. Build green, typecheck clean, 10 MC tests pass, hooks KLC intacts.
  - **2026-07-14 — REBRANDING : SCOPE RÉEL ÉVALUÉ** (ghost `bg_e5cbc346` a loopé/annulé à 150 calls ; scopage fait manuellement par lecture ciblée). Ce n'est **PAS un renommage** : c'est un **remplacement complet du système d'agents** — les 11 agents upstream (Sisyphus/Hephaestus/Atlas/Prometheus/Oracle/Librarian/Explore/Multimodal-Looker/Metis/Momus/Sisyphus-Junior) sont remplacés par 15 agents MaTrix (Morpheus/Neo/Tank/Operator/Oracle/Link/Ghost/Analyst/Keymaker/AgentSmith/Trinity/Cypher/Sentinel/Mouse/Dreamer/Architect). Blast radius : `builtin-agents.ts`, `types.ts` (`BuiltinAgentName`), `config/schema/agent-names.ts`, `builtin-agents/` (`maybeCreateMorpheusConfig/TankConfig/OperatorConfig`), dir `morpheus/` (~15 fichiers prompts modèles ~300KB), tous les `morpheus-*.ts`, `index.ts`, **+ team-mode eligibility registry, canonical-order shim, hooks référençant sisyphus** (`sisyphusJuniorNotepad`, `noSisyphusGpt`, `noHephaestusNonGpt`, `hephaestusAgentsMdInjector`, `atlasHook`) **+ tests associés**. **DÉCISION : A (remplacement complet)** — validé par Shiro sur recommandation Morpheus (objectif « MaTrix autonome » : rebranding = IP fork, non upstreamable). En cours de port (délégué à `tank`, background). Options écartées : (B) rebrand léger — garde le code upstream, trahit l'intention ; (C) additif — conflit canonical-order/team-mode.
  - **2026-07-14 — PORT BLOQUÉ / RÉÉVALUATION SCOPE** : 2 agents (ghost `bg_e5cbc346` puis tank `bg_bda0460b`) ont **loopé à 150 calls**. Le rebranding n'est **PAS contenu** : `diff` legacy vs matrix-port = **457 fichiers divergents hors `agents/`** (cli/, tools/, plugin/, config/, prompts-core/, plugin-handlers/). C'est un **merge complet de fork divergent (79 commits ; legacy en refactor WIP « being ripped apart »)**, pas un port de feature. Tentative (cp agents/ + agent-names + team-mode + shim) laissait 149 fichiers incohérents (réfs sisyphus dans cli/tools/plugin non portées). **RÉVERTI proprement à `ff2973afe`** (KLC/MC intacts, tree clean). **DÉCISION SHIRO REQUISE (scope réévalué)** : (X) merge git legacy→matrix-port + résolution conflits (fidèle, lourd, risqué WIP) ; (Y) rebranding CONTENU = agents/ + agent-names + team-mode + shim + réfs runtime cli/tools/plugin + tests correspondants legacy (livre identité MaTrix, portée moyenne, exécutable par Morpheus en ops déterministes) ; (Z) DIFFÉRER (MaTrix tourne avec noms upstream ; IP KLC/MC/dashboard déjà consolidés). Voir message Morpheus.
