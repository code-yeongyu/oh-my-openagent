## memory/kibitzer: a recall category no connected provider serves is one warning, never a gate-failure streak

`kibitzer/sidecar-model.ts`: an unavailable resolution carries the chain's unconnected `missingProviders`; for `beyond_category` the chain is asked again through `resolveCategory`, because the beyond-category resolution hides why the category came up empty. The starter attaches a `configuration` (category, cause, providers) to the `category_unavailable` / `beyond_category` `KibitzerSidecarStartError`, and `kibitzerConfigurationFailure(error)` reads it back (undefined for `registry_snapshot_unavailable`, `persona_unavailable`, `runtime_unavailable`, `session_create_failed` and plain errors).

`sidecar-outcome.ts` / `sidecar-wake.ts` / `sidecar-turn.ts`: the start-failure end and the wake outcome gain an additive `configuration`; `isDiagnosticWakeEnd` is false for it. Backoff and the carried payload are unchanged, so the next wake re-resolves against the live registry.

`observe.ts` / `observe-record.ts`: a wake with a `configuration` neither counts toward nor resets the diagnostic streak; the first one of a session appends ONE `omo-kibitzer:unavailable` entry, a guard cleared at session shutdown. Category and provider names are masked and bounded (`boundedKibitzerConfiguration`: 128-char category, at most 16 providers of 64 chars) in both the entry and the `wakes.ndjson` line. `notice.ts` renders it fail-closed as a `⚠` warning with the `/login <provider>` and `categories.<name>.model` / `memory.recall.category` fixes; `recall-drain.ts` registers the renderer and `tools/session-read.ts` hides the entry from the sidecar's `session_entries`.
