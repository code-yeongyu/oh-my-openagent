...
| `transcript-archive.ts` | Resolves the bounded transcript-retention archive dir (`<agentHome>/sessions/--omo-senpi-task-archive--`) from `task.transcript_retention` via `resolveAgentHome`; undefined when retention is off, so the engine never receives an archive dir and nothing is written. The archive itself is engine-owned (senpi-task `src/retention/`): per task a 0600 session-shaped metadata manifest `.jsonl` (finder-discoverable) plus a deterministic-gzip `.jsonl.gz` content sidecar, or the manifest alone for metadata-only (protected) selections; the existing Senpi session finder discovers the manifests. |
