...
| `status-ui.ts` / `status-row-format.ts` / `dag-status-ui.ts` | Debounced (250 ms) footer/widget rows for tasks and DAG runs; `store-mutation-observer.ts` triggers sync. DAG paints join each node's current `taskId` through the runtime's task-record reader and reuse the ordinary target formatter for resolved model/effort, including finished nodes. Taskless rows keep their route label. Model detail takes width before activity; width budgeting reserves Senpi's string-widget padding. |
