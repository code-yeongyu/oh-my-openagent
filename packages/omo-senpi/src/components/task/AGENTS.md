...
| `dag-lint.ts` | Advisory lint over a dag definition at the tool boundary (mass-ulw node-prompt doctrine: `TASK:`, `STOP WHEN`, ...). Warnings ride in the `start` result and NEVER reject - the tool stays generic. |
| `dag-fork-source.ts` / `dag-reload-source.ts` | Session-fork provenance read from the previous session file's bounded header, and the live-run view `reload-guard.ts` consumes (only `completed`/`failed`/`cancelled` count as finished). |
...
| `task-skill-loader.ts` | `load_skills` resolution for children: `packagedSkillDirs` covers both layouts (source tree and bundled `plugin/extensions`) and offers `skills-conditional` unconditionally - the PARENT decides what to pass - on top of the home/agent dirs from `resolveAgentHome`; the resolved `git-master` block is then patched with the attribution directive from the `git_master` config section. |
| `judge-runner.ts` | `createInProcessJudgeRunner` - the memory kibitzer judge borrows the task engine's `InProcessRunner`. It is constructed HERE so the omo-task sidecar reaches `runners/in-process` through exactly ONE import path; a second value import made `omo-task.js` emit byte-differently under contended builds. |
| `fast-mode-inheritance.ts` / `category-unavailable-warning.ts` / `usage-guidance.ts` | `-fast` service-tier inheritance from the parent onto a resolved child plan; the once-per-session dead-category warning (`senpi-task.category-unavailable`, with its own renderer); and the once-per-session `<omo-senpi-task>` usage guidance text plus the shared `createOncePerSessionGuard`. |
