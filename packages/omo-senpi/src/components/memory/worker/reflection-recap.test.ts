...
test("#given a replacement during the read #when checked #then changing output is unavailable", async () => {
  ...
  let pathStats = 0
  const report = await readReflectionReport(item.runDir, undefined, async (target) => {
    const stats = await lstat(target)
    pathStats += 1
    if (pathStats === 1) return stats
    return new Proxy(stats, { get: (source, property) => property === "ino" ? source.ino + 1 : Reflect.get(source, property, source) })
  })
  expect(pathStats).toBe(2)
