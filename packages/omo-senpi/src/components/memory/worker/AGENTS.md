...
Detached `senpi -p` reflection/dream/facts child execution and the durable run lifecycle for the memory component: supervision, finalization claims, crash reconciliation, model resolution, health, and completion delivery. Children never touch git; the parent applies writes. Score ~28 (110 files - 98 here plus 12 subprocess fixtures under `__fixtures__/` - own barrel, high export/reference density).
...
| `reservation-run-ledger.ts` / `senpi-command.ts` / `supervisor-process-identity.ts` | Reservation ledger parsing plus worktree derivation, the resolved `senpi`/bun launch command for detached children (`resolveSenpiLaunch`, `resolveMemoryChildLaunch`), and the supervisor's platform primitives: child-exit parsing, `posix`/`win32` detection, clock, absolute-deadline scheduling, process-group signaling. |
| `__fixtures__/*.ts` | Real subprocess fixtures the supervisor/integration tests spawn (reflection and dream children, a lock holder, and supervisors that linger, never publish, or die after writing `outcome.json`). Node built-ins only, so they run under the bare runtime. |
