...
const context: MemoryIdentityContext = createMemoryIdentityContext({
  identity: "agent",
  identityPaths: buildIdentityPaths("/tmp/omo-kibitzer-trigger-latch", "agent"),
  binding: createMemoryBinding({ identity: "agent", repoPath: "/tmp/omo-kibitzer-trigger-latch/repo", boundAt: 0 }),
})
