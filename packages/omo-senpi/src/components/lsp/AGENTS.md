...
| `post-edit-diagnostics.ts` | Post-edit diagnostics over `@oh-my-opencode/lsp-core/post-edit`: mutation tools come from `MUTATION_TOOL_NAMES` in `../post-mutation` (`write`/`edit`/`apply_patch`), concurrency 4, per-session not-configured cache, widget lines under key `omo-senpi-lsp`. |
...
- The one `tool_result` handler runs the FORMATTER before diagnostics: `createFormatterStep` from `../formatter`, configured from `formatOnMutation` in the resolved omo config. Formatter output is appended to the tool result, a formatter error short-circuits with `isError: true`, and only then does the post-edit flag decide whether diagnostics run. `../formatter` and `../post-mutation` are helper modules with no registration of their own - this component is their only live call site.
