...
import { shouldCopySkillSource } from "./sync-skills.mjs"

test("#given a Senpi skill source below an .omo worktree #when its root is filtered #then the skill is copied", () => {
...
test("#given a Senpi skill source below an .omo worktree #when nested .omo state is filtered #then the state is excluded", () => {
  const sourceRoot = join("/tmp", ".omo", "worktrees", "repo", "skills", "example")
...
test("#given .omo as the Senpi skill source root #when the root is filtered #then the state directory is excluded", () => {
