...
Live Senpi QA harness: E2E drivers, continuation probes, scenario fixtures, and mock providers that drive the REAL `senpi` binary against built plugin artifacts. 108 direct driver/support files plus `fixtures/`, `mock-provider/`, `task-14/`, and `thread-tools/`. Unit tests prove wiring; these drivers are the harness proof the root AGENTS.md QA mandate demands. Earned by score: largest and most referenced directory in the package.

## Lanes

| Lane | Drivers |
|------|---------|
| Task | `task-e2e.mjs` (single/batch lifecycles), `task-lane-spill-e2e.mjs`, `task-fallback-notification.mjs`, `task-category-unavailable-e2e.mjs`, `task-id-race-qa.mjs`, `task-parent-restart-e2e.mjs`, `task-summary-e2e.mjs`, `task-load-skills-e2e.mjs`, `task-runtime-fallback-e2e.mjs` (child runtime rotates its model chain), `task-13.test.ts` (engine wiring) |
...
| X-search | `x-search-backtest.mjs` + `-core.mjs` (recall/Jaccard scoring over `fixtures/x-search-backtest`, per-call cost ceiling, secret redaction; `OMO_X_SEARCH_BACKTEST_NO_NETWORK=1` disables `fetch` outright), `x-search-live-e2e.mjs` (credentialed path in a sandboxed agent dir) |
| Isolation | `isolation-state.mjs` - the credential/integrity contract every live lane leans on (`PROTECTED_STATE_FILES`, `OBSERVATION_LIMITS`, `snapshotProtectedState`, `classifyObservedChanges`, `scopedIsolationVerdict`) - with bounded hashing/stat IO in `isolation-file-readers.mjs`, pinned by `isolation-{certification,review-blockers,state-races,platform-capabilities,ultrabrain-blockers}.test.mjs` |
| Infra | `drive.mjs`, `task-e2e-{analysis,process}.mjs`, `mock-completions-server.mjs`, `mock-provider/`, `environment-receipt.ts` (child env receipt written from a senpi-loaded extension) |
