...
| Resume | `task-resume-e2e.mjs`, `task-resume-failure-e2e.mjs`, `task-resume-e2e-scenarios.mjs`, `resume-e2e-runtime.mjs`, `task-child-reconcile-e2e.mjs` (marked RPC child must not recursively restore its persisted child tree) |
