#!/usr/bin/env bun
// allow: SIZE_OK - one auditable offline proof of the kibitzer recall gate: sandbox, mock provider,
// RPC session, judge routing and the parent-session assertions belong to one indivisible lifecycle.
//
// Offline end-to-end proof of the kibitzer recall gate (plan .omo/plans/memorian-judge-completion-policy.md todo 7).
...
async function runScenario(kind, options) {
  ...
  try {
    ...
    const judgeSettled = await waitUntil(
      () => {
        const gate = readEntries(sessionFile).filter((entry) => entry.customType === "omo-kibitzer:gate")
...
function assertS1(entries, sandbox, state, facts, router) {
  const nudged = entries.filter((entry) => entry.type === "custom" && entry.customType === "omo-kibitzer:nudged")
  ...
  const recall = entries.filter((entry) => entry.type === "custom_message" && entry.customType === "omo-kibitzer:recall")
  record("s1.recall-injected", recall.length >= 1, `count=${recall.length}`)

  const failedGate = entries.filter((entry) => entry.customType === "omo-kibitzer:gate" && entry.data?.status === "failed")
...
function assertS2(entries, sandbox, state, facts) {
  const gate = entries.filter((entry) => entry.customType === "omo-kibitzer:gate" && entry.data?.status === "failed")
  ...
  const nudged = entries.filter((entry) => entry.customType === "omo-kibitzer:nudged")
...
async function main() {
  ...
  if (options.evidenceDir !== undefined) {
    ...
    writeFileSync(join(options.evidenceDir, "kibitzer-gate-e2e.json"), `${JSON.stringify(payload, null, 2)}\n`)
    console.log(`evidence: ${join(options.evidenceDir, "kibitzer-gate-e2e.json")}`)
  }
...
function runSelfTest() {
  const judgeBody = { tools: [{ type: "function", function: { name: NUDGE_TOOL } }], messages: [{ role: "system", content: "# Kibitzer — memory nudge agent" }] }
  if (!isJudgeRequest(judgeBody)) throw new Error("self-test: a nudge-tool + persona request must route to the judge")
  if (isJudgeRequest({ tools: [{ type: "function", function: { name: "read" } }], messages: [{ role: "system", content: "# Kibitzer" }] })) {
