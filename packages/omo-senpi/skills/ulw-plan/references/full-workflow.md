...
## Phase 0 - Classify
Size interview depth: **Trivial** (single file, obvious) - one or two confirms, then propose. **Standard** (1-5 files, clear feature/refactor) - full explore + interview/research + Plan Consultant on the complete draft. **Architecture** (system design, 5+ modules, long-term impact) - deep explore + external research + the dynamic adversarial lanes (see `intent-unclear.md`).
...
2. Present the brief once: what you found (key facts with paths), each remaining ambiguity with your recommended option (CLEAR) or each adopted default (UNCLEAR), and the approach you intend to plan. You MUST explain the post-approval sequence: on your okay, I will write the complete plan draft (todos + human TL;DR), run plan-consultant gap analysis against that draft, fold its findings, then run plan-reviewer review rounds (fresh session each, max 5 by default). You MUST explicitly invite opt-out: say so if you do not want the high-accuracy review.
