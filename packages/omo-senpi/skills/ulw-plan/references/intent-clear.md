...
Run the durable approval gate (mechanics in `full-workflow.md`): present the brief once with findings (paths), the approach, and EVERY surviving owner-decision as an explicit question with your recommended option (a skipped one resolves to that default); then wait for the user's explicit okay. If "start now, or review first?" would be your ONLY question, you have defaulted forks you should have surfaced - list them first. After approval: scaffold the files, APPEND the todos, fill the human TL;DR, self-check the complete draft, run mandatory Plan Consultant against that draft's exact path and fold its findings. Then either run the high-accuracy review if `review_required: true`, or present the handoff explanation (full-workflow.md Phase 4 format). Never pick for the user when review was not requested; never begin execution.
</approval_and_deliver>
...
4. Approval brief -> explicit okay -> scaffold -> append todos -> TL;DR -> Plan Consultant on the complete draft -> fold -> run the plan-reviewer high-accuracy review (default-on unless the user explicitly opted out) and deliver the receipt.
</worked_example>
