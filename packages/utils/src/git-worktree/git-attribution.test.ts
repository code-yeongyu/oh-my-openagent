...
describe("git attribution resolution", () => {
  ...
  describe("resolveGitAttribution", () => {
    it("resolves default disabled attribution when no overrides are set", () => {
    ...
    it("resolves auto commit_footer to false when unsuppressed (default off)", () => {
      const result = resolveGitAttribution(
        { commit_footer: "auto" },
        {
          env: {},
    ...
    it("re-enables the builtin footer when OMO_GIT_ATTRIBUTION is 1", () => {
      const result = resolveGitAttribution(
        { commit_footer: false },
        {
          env: { OMO_GIT_ATTRIBUTION: "1" },
      ...
      expect(result.includeCoAuthoredBy).toBe(false)
      expect(result.suppressed).toBe(false)
      expect(result.reason).toBe("env_enabled")
    })

    it("re-enables the builtin footer when git config omo.attribution is true", () => {
      const result = resolveGitAttribution(
        { commit_footer: false },
        {
          env: {},
          gitConfigReader: (key) => (key === "omo.attribution" ? "true" : undefined),
        },
      )
      expect(result.commitFooter).toBe(true)
      expect(result.includeCoAuthoredBy).toBe(false)
      expect(result.suppressed).toBe(false)
      expect(result.reason).toBe("git_config_enabled")
    })

    it("never enables Co-authored-by even when the deprecated flag is true", () => {
      const result = resolveGitAttribution(
        { commit_footer: true, include_co_authored_by: true },
        {
          env: {},
          gitConfigReader: () => undefined,
        },
