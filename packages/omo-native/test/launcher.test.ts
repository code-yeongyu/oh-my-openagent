  ...
  describe("#given a fake senpi package", () => {
    describe("#when the default command is launched", () => {
      ...
      test.skipIf(process.platform !== "win32")("#then the launcher promotes an existing shim back to the first PATH entry", () => {
        ...
        const normalizedShimDirectory = shimDirectory.replace(/\\/g, "/").toLowerCase()
        const inputPath = [join(fixture.root, "other-path-entry"), shimDirectory.replace(/\\/g, "/")].join(";")
        const result = run(fixture, ["say", "hi"], {
          [pathKey]: inputPath,
        }, ["path"])
        expect(result.status, result.stderr).toBe(0)
        ...
        expect(pathEntries.filter((entry) => entry.replace(/\\/g, "/").toLowerCase() === normalizedShimDirectory)).toHaveLength(1)
