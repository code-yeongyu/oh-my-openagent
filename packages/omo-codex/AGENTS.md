...
**Generated:** 2026-09-10 / bee8c2ba4 (update pass; score 13, harness adapter boundary)
...
| `plugin/` | Vendored Codex plugin namespace `omo`; pkg `@sisyphuslabs/omo-codex-plugin` (dep `@oh-my-opencode/shared-skills`). Holds `.codex-plugin/plugin.json` (brandColor `#7C3AED`), the `.codex-plugin/plugin.json` `hooks` array wiring 21 individual hook JSON files in `hooks/` (hand-maintained alongside component `hooks/hooks.json`; build syncs version labels only), `components/` (10 workspaces + bootstrap + test-support + lcx), generated aggregate `skills/` (gitignored, built by sync-skills), `.mcp.json`. |
