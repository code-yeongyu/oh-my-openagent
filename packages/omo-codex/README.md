...
To override LazyCodex-managed Codex agent role models durably, create `~/.codex/omo.toml` (or `$CODEX_HOME/omo.toml` for isolated installs) with `[agents.<role>]` tables. Supported string fields are `model`, `model_reasoning_effort`, and `service_tier`; they are applied only to bundled `omo` agent TOMLs copied into `~/.codex/agents/`.
