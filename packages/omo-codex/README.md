...
The managed subagent concurrency limit defaults to `1000`. Set `LAZYCODEX_SUBAGENT_THREAD_LIMIT` to an integer from `1` through `1000` before installing and when launching Codex to use a smaller cap; the installer and the `SessionStart` migration both apply it. The OMO-prefixed name, `OMO_CODEX_SUBAGENT_THREAD_LIMIT`, is accepted as a compatibility alias, but the `LAZYCODEX_` name takes precedence when both are set. Invalid values use the default.
