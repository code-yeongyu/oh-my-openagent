...
3c. The gate's approval binds to the frozen commit SHA. In an enclosing PR workflow (e.g. work-with-pr), that approval IS the review for any later review gate whose head is the same SHA — never re-run an equivalent review there; only commits pushed after the freeze need fresh review.
