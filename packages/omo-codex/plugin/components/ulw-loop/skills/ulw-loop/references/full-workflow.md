...
| Implementation, LOW difficulty (single-spot fix, boilerplate, config/copy, one-file pattern-following change) | `TASK: act as a low-difficulty implementation worker. ...` — on surfaces exposing `agent_type`, pass `agent_type: "lazycodex-worker-low"` (gpt-5.6-luna/high) |
| Implementation, MEDIUM difficulty (standard feature inside existing layers, a few files, established patterns) | `TASK: act as a medium-difficulty implementation worker. ...` — `agent_type: "lazycodex-worker-medium"` (gpt-5.6-sol/high) when exposed |
| Implementation, HIGH difficulty (new module/abstraction, cross-module refactor, concurrency/security/auth, migration) | `TASK: act as a high-difficulty implementation worker. ...` — `agent_type: "lazycodex-worker-high"` (gpt-5.6-sol/max) when exposed |
...
Difficulty tier (model power) is ORTHOGONAL to the LIGHT/HEAVY rigor tier (process size): HEAVY+low exists (careful process, small change) and LIGHT+high exists (lean process, hard change) — pick each axis on its own facts. Routing note (verified 2026-07-11): the tier roles are installed agent TOMLs selected via `agent_type`; `multi_agent_v1.spawn_agent` exposes `agent_type`, but the deployed `multi_agent_v2` `collaboration.spawn_agent` schema currently does NOT (only `fork_turns`/`message`/`task_name`) — on that surface describe the tier's difficulty and rigor inside `message` and the roles bind automatically once the surface re-exposes `agent_type`.
