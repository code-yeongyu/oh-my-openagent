...
function transportLabel(team) {
	return isMultiAgentV2(team)
		? "MultiAgentV2 (native agents: `agents.spawn_agent` / `agents.send_message` / `agents.followup_task`)"
...
as an \`agents.followup_task\` - switch to it before your next edit.`;
	}
...
function addressBookRules(team) {
	if (isMultiAgentV2(team)) {
		return `- **Reach the leader and your peers with \`agents.send_message\` - not by narrating in your own
  ...
  \`members[].taskName\`).
- **Use \`agents.send_message\` for updates and \`agents.followup_task\` only when handing a peer a NEW task**
...
export function buildGuide(team) {
	const v2 = isMultiAgentV2(team);
	const artifacts =
		team.paths?.artifacts ?? ".omo/teams/<session_id>/artifacts";
...
function pushInstruction(team) {
	if (isMultiAgentV2(team)) {
		return `Push updates to the leader (target \`${LEADER_AGENT_PATH}\`) and relevant peers (their team.json \`members[].agentPath\`) with \`agents.send_message\` as you work`;
	}
