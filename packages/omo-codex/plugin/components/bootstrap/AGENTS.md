...
- **Manifests are committed, never generated at build time.** Builds stay offline and deterministic; the runtime downloader is the explicit network path, while unit tests inject `fetchImpl` and never use the network.
