...
Read-only Codex subagent roles live in `CODEX_HOME/agents/`. Spawn: `multi_agent_v1.spawn_agent({"message":"TASK: act as an explorer. ...","agent_type":"explorer","fork_context":false})`. If your tool list instead has MultiAgentV2 `agents.*` tools: `agents.spawn_agent({"task_name":"<lowercase_digits_underscores>","message":"TASK: act as an explorer. ...","agent_type":"explorer","fork_turns":"none"})`. Omit `model` and `reasoning_effort` so the role TOML controls the child; use `agents.wait_agent({"timeout_ms":...})` for mailbox activity.
