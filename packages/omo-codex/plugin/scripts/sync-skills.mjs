...
// Codex-only guidance relocated from the shared skill source: the shared bundle is loaded verbatim
// by OpenCode, so tier mapping and Codex subagent reliability are injected here for Codex only.
export const ulwExecuteCodexReliabilityGuidance = `### Codex tier mapping for the delegation router
...
export function applyCodexSkillOverlays(skillName, content) {
	...
	if (skillName === "ulw-execute") {
		const withCodexReliability = content.includes(ulwExecuteCodexReliabilityGuidance)
			? content
			: content.replace(codexHarnessToolCompatibility, `${codexHarnessToolCompatibility}${ulwExecuteCodexReliabilityGuidance}`);
		return withCodexReliability
			.replace(ulwExecuteOriginalCompletion, ulwExecuteCodexCompletion)
			.replace(ulwExecuteOriginalHardRule, ulwExecuteCodexHardRule)
			.replace(/<platform>:<session_id>/g, "codex:<session_id>");
	}
