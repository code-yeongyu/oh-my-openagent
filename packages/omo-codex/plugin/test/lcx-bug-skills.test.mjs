...
test("#given synced lcx-report-bug skill #when inspected #then it files LazyCodex bug issues with generated labels", async () => {
	...
	assert.match(skill, /code-yeongyu\/oh-my-openagent/);
	assert.match(skill, /packages\/omo-codex/);
	assert.match(skill, /packages\/shared-skills/);
	...
	assert.match(skill, /TARGET_REPO="code-yeongyu\/oh-my-openagent" # or openai\/codex/);
	...
	assert.match(skill, /Global Review and Debugging Gate/);
	assert.match(skill, /\breview-work\b/);
	assert.match(skill, /debugging-oriented runtime audit/);
	assert.match(skill, /inconclusive lanes block the PR/);
	assert.match(skill, /subagent prompts, repro artifacts, logs, PR bodies, comments, or handoffs/);
	assert.match(skill, /raw tokens, credentials, auth headers, cookies, API keys, env dumps, private logs, or PII/);
	assert.ok(
		skill.indexOf("Before any branch handoff, PR creation, or PR handoff") <
			skill.indexOf("gh pr create --repo \"$TARGET_REPO\""),
		"lcx-report-bug must gate before PR creation",
	);
...
test("#given synced lcx-contribute-bug-fix skill #when inspected #then it contributes LazyCodex bug-fix PRs from fresh temp workspaces", async () => {
	...
	assert.match(skill, /packages\/omo-codex/);
	assert.match(skill, /packages\/shared-skills/);
	assert.match(skill, /code-yeongyu\/lazycodex` only for downstream marketplace/);
	...
	assert.match(skill, /subagent prompts, repro artifacts, logs, PR bodies, comments, or handoffs/);
	assert.match(skill, /raw tokens, credentials, auth headers, cookies, API keys, env dumps, private logs, or PII/);
	assert.match(skill, /Global Review and Debugging Gate PASS evidence/);
