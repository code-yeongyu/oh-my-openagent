...
describe("createTeam failures", () => {
  test("#given creator rollback owns cleanup #when startup recovery overlaps after lease expiry #then only the creator tears members down", async () => {
    // given
    ...
    const manager = new FakeTeamManager({
      behaviors: [{ kind: "ok" }, { kind: "throw", message: "spawn failed" }],
      beforeCancel: async () => {
    ...
    }, "creator-recovery-race")

    // when
    const creation = createTeam(spec, "project", {
      manager,
      stateDir,
    ...
    const recovery = await recoverStaleCreatingTeams({
      manager,
      stateDir,
      taskSettings: settings,
      now: () => Date.now() + CREATE_CLEANUP_LEASE_TTL_MS,
    })

    // then
    expect(recovery).toEqual({ markedFailed: 0, errors: [] })
  ...
  test("#given failed-state persistence rejects #when immediate startup recovery runs #then its retained cleanup claim completes finalization", async () => {
    const stateDir = stateDirConfig(tempProjectDir())
    const settings = taskSettings({ max_parallel_members: 1 })
    ...
    const teamRunId = manager.started[0]?.name?.split(":")[1] ?? ""
    const config = toTeamCoreConfig(settings, teamStorageBaseDir(stateDir))
    expect((await loadRuntimeState(teamRunId, config)).status).toBe("create_cleanup_pending")
  ...
  test("#given terminal destruction returns without changing residency #when rollback verifies cleanup #then its journal and cleanup state remain retryable", async () => {
    const stateDir = stateDirConfig(tempProjectDir())
    const settings = taskSettings({ max_parallel_members: 1 })
    const manager = new FakeTeamManager({ behaviors: [{ kind: "ok", status: "completed" }, { kind: "throw", message: "later failure" }] })
    const attempt = createTeam(normalizeSenpiTeamSpec({
  ...
  test("#given rollback cancellation rejects #when member creation fails #then the original error survives and cleanup state remains retryable", async () => {
    // given
    const stateDir = stateDirConfig(tempProjectDir())
    const settings = taskSettings({ max_parallel_members: 1 })
