...
export async function stageAstGrepMcpRuntime(options = {}) {
  ...
  const tempParent = await mkdtemp(join(dirname(targetDir), ".tmp-omo-omp-ast-grep-runtime-"))
...
async function validateEntry(entry, kind) {
  ...
  try {
    ...
    if (isErrno(error, "ENOENT")) throw new Error(`OMP ast-grep MCP ${kind} CLI is missing: ${entry}`)
    throw error
  }
  if (!entryStat.isFile()) throw new Error(`OMP ast-grep MCP ${kind} CLI is not a file: ${entry}`)
  ...
  try {
    if (process.argv.includes("--check")) {
      ...
      if (!(await fileExists(defaultSourceEntry))) {
        ...
        console.log(`OMP ast-grep-mcp runtime is current: ${result.targetEntry} sha256=${result.sha256}`)
      }
    } else {
      const result = await stageAstGrepMcpRuntime()
      console.log(`Staged OMP ast-grep MCP runtime: ${result.targetEntry}`)
    }
  } catch (error) {
