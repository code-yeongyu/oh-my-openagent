...
export async function stageAgentToolkit(options = {}) {
  ...
  const tempParent = await mkdtemp(join(dirname(targetDir), ".tmp-omo-omp-agent-toolkit-"))
...
async function probeSelfContainment(targetDir) {
  const cwd = await mkdtemp(join(tmpdir(), "omo-omp-agent-toolkit-probe-"))
...
async function validateFile(path, label) {
  ...
  try {
    ...
    if (isErrno(error, "ENOENT")) throw new Error(`OMP ${label} is missing: ${path}`)
    throw error
  }
  if (!value.isFile()) throw new Error(`OMP ${label} is not a file: ${path}`)
}
  ...
  try {
    if (process.argv.includes("--check")) {
      ...
      if (!(await fileExists(defaultSourceEntry))) {
        ...
        console.log(`OMP agent-toolkit runtime is current: ${result.targetDir} sha256=${result.sha256}`)
      }
    } else {
      const result = await stageAgentToolkit()
      console.log(`Staged OMP agent-toolkit runtime: ${result.targetDir}`)
    }
  } catch (error) {
