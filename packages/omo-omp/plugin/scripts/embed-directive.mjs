...
// harness surfaces omp does not have; any match means other-edition text leaked in,
...
export function transformDirective(rawSkill) {
  ...
  if (violations.length > 0) {
    throw new Error(`omp ultrawork directive source contains forbidden non-omp tokens:\n  - ${violations.join("\n  - ")}`)
  }
...
function renderGeneratedModule(directive) {
  return [
    ...
    "] as const",
    "",
    `export const OMP_ULTRAWORK_DIRECTIVE = ${JSON.stringify(directive)} as const`,
    "",
  ].join("\n")
}
