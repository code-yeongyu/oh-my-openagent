...
export async function syncVersion(options = {}) {
  ...
  if (typeof version !== "string" || version.length === 0) {
    throw new Error("Cannot sync omo-omp plugin version: adapter package.json has no version")
  }
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!result.ok) {
    console.error(`omo-omp plugin/package.json version mismatch: expected ${result.version}, found ${result.current}`)
    process.exit(1)
  }
  console.log(`omo-omp plugin/package.json version ${result.version}${result.changed ? " synced" : " current"}`)
}
