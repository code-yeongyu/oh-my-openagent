...
export async function stageLspDaemonRuntime(options = {}) {
  ...
  const tempParent = await mkdtemp(join(dirname(targetDist), ".tmp-omo-omp-lsp-runtime-"))
  ...
  try {
    ...
    if (process.env.OMO_OMP_STAGE_FAIL_AFTER_BACKUP === "1") {
      throw new Error("Injected OMP runtime staging failure after backup")
    }
...
export async function verifyRuntimeDist(distDir) {
  ...
  if (actual !== expected) {
    throw new Error(`OMP LSP runtime manifest output hash mismatch at ${join(dist, manifestName)}`)
  }
...
export async function checkRuntimeDistFresh(options = {}) {
  ...
  if (manifest.inputDigest !== expectedInputDigest) {
    throw new Error(`OMP LSP runtime manifest is stale at ${join(targetDist, manifestName)}`)
  }
...
async function readRuntimeManifest(distDir) {
  ...
  if (schemaVersion !== 1) throw new Error(`OMP LSP runtime manifest schemaVersion must be 1: ${manifestPath}`)
  if (typeof version !== "string" || version.length === 0) throw new Error(`OMP LSP runtime manifest version is invalid: ${manifestPath}`)
  if (typeof inputDigest !== "string" || !/^sha256:[0-9a-f]{64}$/.test(inputDigest)) {
    throw new Error(`OMP LSP runtime manifest inputDigest is invalid: ${manifestPath}`)
  }
  if (!Array.isArray(outputs)) throw new Error(`OMP LSP runtime manifest outputs must be an array: ${manifestPath}`)
  for (const output of outputs) {
    if (!isRecord(output) || typeof output.path !== "string" || typeof output.sha256 !== "string") {
      throw new Error(`OMP LSP runtime manifest output entry is invalid: ${manifestPath}`)
    }
    if (!/^[0-9a-f]{64}$/.test(output.sha256)) {
      throw new Error(`OMP LSP runtime manifest output hash is invalid: ${manifestPath}`)
    }
  }
  const sorted = [...outputs].sort((left, right) => left.path.localeCompare(right.path))
  if (JSON.stringify(sorted) !== JSON.stringify(outputs)) {
    throw new Error(`OMP LSP runtime manifest outputs must be sorted: ${manifestPath}`)
  }
...
async function validateRequiredOutputs(distDir) {
  for (const output of REQUIRED_OUTPUTS) {
    ...
    try {
      ...
      if (isErrno(error, "ENOENT")) throw new Error(`OMP LSP runtime is missing ${output} at ${distDir}`)
      throw error
    }
    if (!stats.isFile()) throw new Error(`OMP LSP runtime output is not a file: ${path}`)
  }
}
  ...
  try {
    if (process.argv.includes("--check")) {
      ...
      if (!(await fileExists(defaultSourceDist))) {
        ...
        console.log(`OMP LSP runtime is current: ${defaultTargetDist}`)
      }
    } else {
      const result = await stageLspDaemonRuntime()
      console.log(`Staged OMP LSP runtime: ${result.targetDist}`)
    }
  } catch (error) {
