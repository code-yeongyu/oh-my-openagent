...
import { resolveOmpCommentCheckerBinary } from "./resolver"
...
export function createCommentCheckerComponent(options: CommentCheckerComponentOptions = {}): OmoOmpComponent {
...
function ensureBinaryPath(resolveBinary: () => string | null, state: BinaryResolutionState): string | null {
  ...
  try {
    ...
    state.logger.warn("omo-omp comment-checker binary resolution failed; component disabled for this session", { error })
    nextBinaryPath = null
  }
  ...
  if (nextBinaryPath === null) {
    ...
    if (!state.missingBinaryNoticeLogged) {
      state.logger.warn("omo-omp comment-checker binary unavailable; component disabled for this session")
...
function defaultResolveBinary(): string | null {
  return resolveOmpCommentCheckerBinary()
}
