...
describe("runOmpStartupMigration", () => {
  test("#given both legacy source groups without an OpenCode boot #when Senpi starts #then the core migration engine migrates both into the unified config", () => {
    ...
    // when
    const result = withProcessPlatform("win32", () => runOmpStartupMigration(migrationOptions(fileSystem)))
  ...
  test("#given OpenCode and Senpi race on the first legacy migration #when Senpi holds the injected core lock #then exactly one caller migrates", () => {
    ...
    // when
    const senpiResult = withProcessPlatform("win32", () => runOmpStartupMigration({
      ...options,
      onBoundary: (boundary) => {
...
describe("createConfigStartupComponent", () => {
  test("#given migration results and config diagnostics #when session_start captures a UI #then it reports both once through that UI", async () => {
    ...
    expect(notifications).toEqual([
      {
        message: "omo-omp: migrated legacy configuration from /home/alice/.config/opencode/oh-my-openagent.jsonc",
        type: "info",
      },
      { message: "omo-omp: configuration diagnostics: JSONC parse error", type: "warning" },
    ])
    expect(logs).toEqual([])
  })
  ...
  test("#given a migration target with a conflicting legacy value #when session_start captures a UI #then it reports the skipped-conflict diagnostic once", async () => {
    ...
    const migration = runOmpStartupMigration(migrationOptions(fileSystem))
    const pi = new FakeExtensionAPI()
    const logs: string[] = []
    createConfigStartupComponent({
    ...
    expect(notifications).toEqual([
      ...
      {
        message: "omo-omp: configuration migration: skipped: [opencode].agents.finder.model legacy=\"provider/legacy\" kept=\"provider/kept\"",
        type: "warning",
      },
    ])
    expect(logs).toEqual([])
  ...
  test("#given config diagnostics without a UI #when session_start fires #then it logs a single fallback warning", async () => {
    ...
    // then
    expect(logs).toEqual(["warn:omo-omp: configuration diagnostics: Invalid omo config"])
  })
})
