...
const ULTRAWORK_DISABLED_FLAG = "omo-omp-ultrawork-disabled"
...
type OmpInputEventResult = { action: "continue" } | { action: "transform"; text: string }
...
function isCurrentArmingSlot(value: unknown): value is SharedArmingSlot {
  ...
  return slot.directive === OMP_ULTRAWORK_DIRECTIVE && typeof slot.arming === "object" && slot.arming !== null
}
...
export function sharedSessionArming(): SessionArming {
  ...
  const slot: SharedArmingSlot = { directive: OMP_ULTRAWORK_DIRECTIVE, arming: created }
  registry[ARMING_LEDGER_KEY] = slot
  return created
}

export function createUltraworkComponent(arming: SessionArming = sharedSessionArming()): OmoOmpComponent {
  return {
    name: "ultrawork",
    register(pi: OmpExtensionAPI, ctx: ComponentContext): void {
      pi.on("input", (payload: unknown, eventCtx: unknown): OmpInputEventResult =>
...
export function classifyUltraworkInput(
  input: { readonly text: string; readonly source: OmpInputEvent["source"] },
...
function armUltrawork(
  ...
  const content = arming.isArmed(sessionId) ? ULTRAWORK_REMINDER : OMP_ULTRAWORK_DIRECTIVE
  arming.markArmed(sessionId)
...
function isOmpInputEvent(value: unknown): value is OmpInputEvent {
