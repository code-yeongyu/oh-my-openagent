...
import { FORBIDDEN_DIRECTIVE_TOKENS, OMP_ULTRAWORK_DIRECTIVE } from "./generated-directive"
...
const generatedDirectivePath = resolve("packages/omo-omp/src/extension/components/ultrawork/generated-directive.ts")

describe("omo-omp ultrawork component", () => {
  ...
  it("#given ultrawork disabled flag #when trigger input dispatches #then suppresses injection", async () => {
    // given
    const pi = new FakeExtensionAPI()
    pi.setFlag("omo-omp-ultrawork-disabled", true)
    await registerIsolatedUltrawork(pi)
  ...
  it("#given embedded directive #when inspected #then contains zero forbidden non-senpi tokens", () => {
    ...
    for (const token of FORBIDDEN_DIRECTIVE_TOKENS) {
      expect(OMP_ULTRAWORK_DIRECTIVE.toLowerCase()).not.toContain(token.toLowerCase())
    }
  })

  it("#given embedded directive #when inspected #then keeps required ultrawork anchors", () => {
    // then
    expect(OMP_ULTRAWORK_DIRECTIVE).toContain("ULTRAWORK MODE ENABLED!")
    expect(OMP_ULTRAWORK_DIRECTIVE).toMatch(/# Tier triage/i)
    expect(OMP_ULTRAWORK_DIRECTIVE).toMatch(/Evidence-driven|captured evidence|evidence/i)
    expect(markerCount(OMP_ULTRAWORK_DIRECTIVE)).toBe(1)
  })
  ...
  it("#given embedded directive #when inspected #then keeps the senpi-native tool contract", () => {
    ...
    expect(OMP_ULTRAWORK_DIRECTIVE).toContain("create_goal")
    expect(OMP_ULTRAWORK_DIRECTIVE).toContain("`todo`")
    expect(OMP_ULTRAWORK_DIRECTIVE).toContain("team_create")
    expect(OMP_ULTRAWORK_DIRECTIVE).toContain("# Parallel execution")
    expect(OMP_ULTRAWORK_DIRECTIVE).toContain("# Stop rules")
  })
  ...
  it.skip("#given generated directive drift #when embed script runs check #then fails", () => {
    // given: same embed-directive.mjs dependency as the sibling drift test; skipped until the
    // omo-omp plugin pipeline lands it.
