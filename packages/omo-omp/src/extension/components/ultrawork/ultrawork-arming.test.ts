...
import { OMP_ULTRAWORK_DIRECTIVE } from "./generated-directive"
...
describe("omo-omp ultrawork once-per-session arming", () => {
  it("#given an armed session #when a second trigger dispatches #then injects a short reminder instead of the full directive", async () => {
    ...
    expect(pi.messages[0]?.message["content"]).toBe(OMP_ULTRAWORK_DIRECTIVE)
  ...
  it("#given an armed session #when the session compacts #then the next trigger injects the full directive again", async () => {
    ...
    expect(result).toEqual({ action: "continue" })
    expect(pi.messages).toHaveLength(2)
    expect(pi.messages[1]?.message["content"]).toBe(OMP_ULTRAWORK_DIRECTIVE)
  })
  ...
  it("#given one live session #when triggers span rejected and accepted compactions #then the directive-reminder sequence stays pinned", async () => {
    ...
    expect(pi.messages[3]?.message["content"]).toBe(OMP_ULTRAWORK_DIRECTIVE)
  })
  ...
  it("#given suppression routes #when matching text dispatches #then no message emits and the session ledger stays unarmed", async () => {
    ...
    for (const testCase of cases) {
      ...
      const pi = new FakeExtensionAPI()
      const arming = createSessionArming()
      pi.setFlag("omo-omp-ultrawork-disabled", testCase.disabled)
      await createUltraworkComponent(arming).register(pi, createTestContext(pi))
  ...
  it("#given shared arming state #when snapshot is read three times #then every read is identical and the ledger stays unchanged", () => {
    ...
    const restoreSlot = seedSharedArmingSlot({ directive: OMP_ULTRAWORK_DIRECTIVE, arming })
  ...
  it("#given an armed session #when the component re-registers on resume #then the next trigger is the reminder not the full directive", async () => {
    ...
    expect(piBeforeResume.messages[0]?.message["content"]).toBe(OMP_ULTRAWORK_DIRECTIVE)
  ...
  it.skip("#given the packaged extension reloaded uncached #when the same session triggers on the second load #then injects the reminder not the full directive", async () => {
    // given: this test dynamically imports the BUILT packaged extension bundle
    // (plugin/extensions/omo.js). The omo-omp bundle is produced by the plugin build pipeline
    // (parallel work) and does not exist yet, so the import cannot resolve; skipped until it does.
    ...
    expect(firstInjection?.message["content"]).toBe(OMP_ULTRAWORK_DIRECTIVE)
