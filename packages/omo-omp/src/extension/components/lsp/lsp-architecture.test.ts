...
const COMPONENT_ROOT = "packages/omo-omp/src/extension/components/lsp"
...
const FORBIDDEN_ACTIVE_PATTERNS = [
  ["vscode", "jsonrpc"].join("-"),
  ["OMO", "OMP", "TRUST", "PROJECT", "LSP", "COMMANDS"].join("_"),
  ["OMP", "TRUST", "PROJECT", "LSP", "COMMANDS"].join("_"),
] as const

describe("omo-omp lsp architecture boundary", () => {
  test("#given the daemon-backed adapter #when source shape is audited #then no vendored LSP engine remains", () => {
    ...
    const activePatternHits = collectPatternHits([
      "packages/omo-omp/package.json",
      "packages/omo-omp/plugin/package.json",
      ...sourceFiles,
    ])
