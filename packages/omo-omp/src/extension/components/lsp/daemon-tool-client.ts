...
import { resolveOmpPackagedDaemonRuntime } from "./daemon-runtime.js";
...
type OmpLspToolName = keyof typeof DAEMON_TOOL_NAMES;

class UnsupportedOmpLspToolError extends Error {
	constructor(readonly toolName: string) {
		super(`Unsupported Omp LSP daemon tool: ${toolName}`);
		this.name = "UnsupportedOmpLspToolError";
	}
}
...
export async function callPackagedDaemonTool(
	...
	const context = currentOmpRequestContext();
...
function currentOmpRequestContext(): LspRequestContext {
...
function toDaemonToolName(name: string): string {
	if (isOmpLspToolName(name)) return DAEMON_TOOL_NAMES[name];
	throw new UnsupportedOmpLspToolError(name);
}

function isOmpLspToolName(name: string): name is OmpLspToolName {
...
async function loadPackagedDaemonClient(importerUrl: string): Promise<DaemonClientModule> {
	const runtime = resolveOmpPackagedDaemonRuntime(importerUrl);
