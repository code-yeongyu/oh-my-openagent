...
export interface OmpDaemonRuntime {
  readonly cliPath: string
  readonly version: string
}

export function resolveOmpDaemonRuntime(
  env: Record<string, string | undefined> = process.env,
  packagedRuntime: OmpDaemonRuntime = resolveOmpPackagedDaemonRuntime(),
): OmpDaemonRuntime {
  ...
  if (!hasCliOverride || !hasVersionOverride) {
    if (!isAbsolute(packagedRuntime.cliPath)) {
      throw new Error("Packaged Omp LSP daemon CLI path must be absolute")
    }
...
export function resolveOmpPackagedDaemonRuntime(importerUrl: string = import.meta.url): OmpDaemonRuntime {
  ...
  if (!existsSync(cliPath)) throw new Error(`Omp packaged LSP daemon CLI is missing: ${cliPath}`)
  const parsed: unknown = JSON.parse(readFileSync(packageJsonPath, "utf8"))
  if (!isRecord(parsed) || typeof parsed.version !== "string" || parsed.version.length === 0) {
    throw new Error(`Omp packaged LSP daemon version is missing: ${packageJsonPath}`)
  }
