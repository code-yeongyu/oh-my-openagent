...
import type { ComponentContext, ComponentLogger, OmpExtensionAPI } from "../../types"
...
export interface MemoryWiringOptions {
  ...
  readonly loadConfig: (options: { readonly cwd?: string }) => OmpOmoConfigResult
  readonly cwd: () => string
...
export interface MemoryWiring {
  registerStatic(pi: OmpExtensionAPI, ctx: ComponentContext): void
  clearStatus(eventCtx: unknown): void
  afterBind(pi: OmpExtensionAPI, sessionId: string, identity: MemoryIdentityContext, eventCtx: unknown): Promise<void>
}
...
export function createMemoryWiring(options: MemoryWiringOptions): MemoryWiring {
  ...
  function completionApi(pi: OmpExtensionAPI): ReflectionCompletionApi | undefined {
  ...
  return {
    ...
    async afterBind(pi: OmpExtensionAPI, sessionId: string, identity: MemoryIdentityContext, eventCtx: unknown): Promise<void> {
      activeSessionId = sessionId
      lastEventCtx.current = eventCtx
