...
import type { OmoAgentToolResult, OmoToolDefinition } from "../../types"
import {
...
// The agent loop honors an inline `isError` on the returned result (harness builtin tool
// convention); the base OmoAgentToolResult type does not declare it, so it is intersected on here.
export type MemoryToolExecutionResult = OmoAgentToolResult<MemoryToolResultDetails> & { readonly isError?: boolean }

export type MemoryToolDefinition<TParams extends TSchema> = Omit<OmoToolDefinition, "execute" | "parameters"> & {
