...
import type { ComponentLogger, OmpExtensionAPI } from "../../types"
...
export interface ReflectionTriggerWiring {
  register(pi: OmpExtensionAPI): void
...
export function createReflectionTriggerWiring(options: ReflectionTriggerWiringOptions): ReflectionTriggerWiring {
  ...
  function track(task: () => Promise<void>): void {
    const promise = task()
      .catch((error: unknown) => {
        options.logger?.warn("omo-omp memory reflection trigger failed", { error: describe(error) })
      })
      .finally(() => {
  ...
  function launch(request: ReflectionRequest): void {
    try {
      ...
      options.logger?.warn("omo-omp memory reflection launch failed", { error: describe(error) })
    }
  }
