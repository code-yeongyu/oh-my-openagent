...
// Structural mirror of the harness filesystem policy API (senpi branch feat/extension-fs-policy:
...
// local so omo-omp keeps compiling against harness releases that predate the API.
...
export function hasFilesystemPolicySupport(
  api: OmpExtensionAPI,
): api is OmpExtensionAPI & FilesystemPolicyRegistration {
  const candidate: { on: OmpExtensionAPI["on"]; registerFilesystemPolicy?: unknown } = api
