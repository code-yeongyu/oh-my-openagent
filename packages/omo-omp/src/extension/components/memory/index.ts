...
const GLOBAL_DISABLED_FLAG = "omo-omp-disabled"
const MEMORY_DISABLED_FLAG = "omo-omp-memory-disabled"
...
export function createMemoryComponent(options: MemoryComponentOptions = {}): OmoOmpComponent {
  ...
  return {
    ...
    register(pi: OmpExtensionAPI, ctx: ComponentContext): void {
      ...
      if (missing.length > 0 || !hasMemoryCapabilities(pi)) {
        ctx.logger.warn("omo-omp memory component skipped: missing ExtensionAPI capabilities", { missing })
        return
      }
...
export function resolveMemoryConfig(loaded: OmpOmoConfigResult): ResolvedMemoryConfig {
