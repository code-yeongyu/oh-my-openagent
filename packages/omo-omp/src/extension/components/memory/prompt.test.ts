...
import type { OmoBeforeAgentStartEventResult } from "../../types"
...
async function dispatchEvent(
  ...
  payload: unknown,
  ctx: unknown,
): Promise<OmoBeforeAgentStartEventResult | undefined> {
  const results = await pi.dispatch("before_agent_start", payload, ctx)
  return results[0] as OmoBeforeAgentStartEventResult | undefined
}
...
describe("createMemoryPromptHandler", () => {
  ...
  test("#given another extension already rewrote the system prompt #when the handler runs #then the foreign text survives and the block is appended", async () => {
    ...
    const foreignPrompt = (foreign as OmoBeforeAgentStartEventResult).systemPrompt ?? ""
