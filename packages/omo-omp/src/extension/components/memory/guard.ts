...
import type { ComponentContext, OmpExtensionAPI, OmoToolCallEventResult } from "../../types"
...
const BASH_ADVISORY = "omo-omp memory guard advisory: bash command references another memory identity; shell text is not blocked because this soft guard is not a security boundary"
...
export function registerMemoryGuard(
  ...
  pi.on("tool_call", (payload, eventContext): OmoToolCallEventResult | undefined => {
