    ...
    register(pi: OmpExtensionAPI, ctx: ComponentContext): void {
      ...
      pi.on("agent_end", async (_payload, eventCtx) => {
        if (state.consecutiveContinuations >= CONTINUATION_LIMIT) {
          ctx.logger.info("omo-omp start-work-continuation skipped", {
        ...
        const cwd = extractCwd(eventCtx)
        if (!sessionId || !cwd) {
          ctx.logger.info("omo-omp start-work-continuation skipped", { reason: "missing-context" })
          return
        }
        ...
        if (!continuable) {
          state.lastSignature = undefined
          ctx.logger.info("omo-omp start-work-continuation skipped", { reason: "not-continuable" })
          return
        }
        ...
        if (state.lastSignature === signature) {
          ctx.logger.info("omo-omp start-work-continuation skipped", { reason: "stale-signature" })
          return
        }
...
const START_WORK_CONTINUATION_INJECTION_KEY = "omo-omp-start-work-continuation"

function deliverContinuation(pi: OmpExtensionAPI, ctx: ComponentContext, content: string): void {
  if (ctx.idleCoordinator !== undefined) {
    ctx.idleCoordinator.enqueue({
      ...
      customType: "omo-omp:start-work-continuation",
      content,
      display: false,
    })
...
function renderDirective(state: DirectiveState): string {
  ...
  return [
    "<omo-omp-start-work-continuation>",
