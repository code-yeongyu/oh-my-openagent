...
/**
 * Where the omp engine keeps its agent state for the install we are attached to.
 *
 * The branded omo distribution stores that state FLAT under `~/.omo`, while a standalone
 * engine keeps it under `~/.omp/agent`. Both layouts can exist on the same machine during
 * a transition, so the location is resolved rather than assumed, and the environment
...
export const AGENT_HOME_SENTINEL = "config.yml"
