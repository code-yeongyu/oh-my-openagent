...
import type { OmoMessageRenderer } from "./renderers"
...
// Minimal structural stand-in for the harness Theme the renderers receive (senpi Theme parity). The
// renderers never read theme members (linesComponent renders plain text), so a record-shaped stub
// satisfies the `theme?: unknown` renderer seam.
class Theme {
  constructor(
    readonly fg: Record<string, string>,
    readonly bg: Record<string, string>,
...
describe("task-family custom message renderers", () => {
  ...
  test("#given a completed team member with a long target #when rendering at 140 cells #then minute duration tools and tps remain visible", () => {
    ...
    // then
    expect(text).toContain("name:stats-member")
    expect(text).toContain("status:completed")
