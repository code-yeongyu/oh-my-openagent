...
// The adapter keeps the harness entry type-only, so the stub tools below carry a local schema
// instead of borrowing one from a runtime harness export.
const sampleParameters: OmoToolDefinition["parameters"] = {
  type: "object",
...
function makeTool(name: string): OmoToolDefinition {
  return {
    name,
    label: name,
...
describe("task child memory tool exclusion", () => {
  ...
  test("#given shared parent tools including memory tools #when an in-process child starts #then the child tool set excludes them", async () => {
    // given
    let captured: { readonly customTools?: readonly { name: string }[] } | undefined
    const fake = createFakeSession()
    const runner = new InProcessRunner({
      // The omp captured tools (OmoToolDefinition) flow into the senpi-task runner's shared-tool
      // seam; the runner only filters them by name, so the local stub tools pass through the runner's
      // typed boundary via the same shape-only cast the production wiring uses.
      sharedParentTools: [makeTool("grep"), makeTool("memory"), makeTool("memory_apply_patch")] as unknown as NonNullable<ConstructorParameters<typeof InProcessRunner>[0]>["sharedParentTools"],
