...
import type { OmoAgentToolResult } from "../../extension/types"
...
export async function runMemberTaskSend(
...
): Promise<OmoAgentToolResult<MemberTaskSendDetails>> {
  ...
  // senpi-task's toolResult returns the senpi AgentToolResult record shape; the omp contract's
  // OmoAgentToolResult is the same structural shape (text content + details), so the shape-only
  // assertion lands it through the local result type without importing a harness tool-result type.
  return toolResult(`Message enqueued to ${input.to} (id: ${message.messageId}).`, {
    kind: "team_message",
    message_id: message.messageId,
    to: input.to,
  }) as unknown as OmoAgentToolResult<MemberTaskSendDetails>
}

// Record-shaped member send tool (senpi defineTool parity): name/description/parameters/execute is
// the contract's OmoToolDefinition shape, registered through the omp registerTool(Record) seam.
export function createMemberTaskSendTool(
  ...
  return {
    ...
    execute: (_toolCallId: string, params: MemberTaskSendInput) => runMemberTaskSend(deps, params),
  }
}
