...
import { createTaskRecordStore } from "@oh-my-opencode/senpi-task"
import type { OmpExtensionAPI } from "../../extension/types"
...
// Task id shape parity with senpi-task's state id (st_[0-9a-f]{8}); the senpi-task root does not
// re-export parseTaskId, so the member extension carries a minimal local validator for the same
// grammar. Type-only import of the TaskId literal type stays off the package surface by spelling it
// inline.
type MemberTaskId = `st_${string}`

const TASK_ID_PATTERN = /^st_[0-9a-f]{8}$/

function parseMemberTaskId(value: string): MemberTaskId {
  if (!TASK_ID_PATTERN.test(value)) throw new Error("Invalid task id; expected st_[0-9a-f]{8}")
  return value as MemberTaskId
}

export type ParsedMemberExtensionEnv = {
  readonly teamRunId: string
  readonly memberName: string
  readonly taskId: MemberTaskId
...
const activeRuntimes = new WeakMap<OmpExtensionAPI, ActiveRuntime>()
...
export function parseMemberExtensionEnv(env: NodeJS.ProcessEnv): ParsedMemberExtensionEnv {
  ...
  let taskId: MemberTaskId
  try {
    taskId = parseMemberTaskId(taskIdRaw)
  } catch (error) {
...
export default async function registerMemberExtension(pi: OmpExtensionAPI): Promise<void> {
...
function stopRuntime(pi: OmpExtensionAPI, runtime: ActiveRuntime): void {
  runtime.started = false
...
function runSafely(operation: string, promise: Promise<void>): void {
  promise.catch((error: unknown) => {
    log("omo-omp member extension poll failed", { operation, error: String(error) })
  })
}
