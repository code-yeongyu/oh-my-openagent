#!/usr/bin/env node
// Standalone stdio MCP server exposing the omo memory tools. The omp extension registers this server
// with exposure "search" so the tools surface through the harness's tool_search catalog instead of
// occupying the always-on tool set. It runs under plain Node with no harness runtime: identity
// derives from the child process cwd (the session project directory) via memory-core's auto rule.
...
import { prepareMemoryEngineSession } from "../extension/components/memory/engine-session"
import {
  MEMORY_APPLY_PATCH_DESCRIPTION,
  MEMORY_APPLY_PATCH_TOOL_NAME,
  MEMORY_TOOL_DESCRIPTION,
  MEMORY_TOOL_NAME,
} from "../extension/components/memory/tool-metadata"
