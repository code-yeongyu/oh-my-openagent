...
export function createOpenAIOfficialProvider(creds: ProviderCredentials): ProbeProvider {
  ...
  return {
    ...
    refreshCredentials: async (refreshType: string): Promise<CredentialRefreshResult> => ({
      ...
      message: "openai uses static API keys; no refresh",
    }),
...
function mapError(outcome: { ok: boolean; status: number | null; response_body: string | null; error_message?: string }): ProbeError | undefined {
  ...
  if (outcome.status === 429) {
    return { kind: "rate_limited", message: "openai rate-limited", http_status: 429, retryable: true }
  }
  if (outcome.status === 401 || outcome.status === 403) {
    return {
      kind: "blocked",
      message: outcome.status === 401 ? "openai unauthorized" : "openai forbidden",
      http_status: outcome.status,
      retryable: false,
    }
  }
  if (outcome.status >= 500) {
    return { kind: "http_error", message: `openai ${outcome.status}`, http_status: outcome.status, retryable: true }
  }
  return undefined
}
