...
const ERROR_TAXONOMY: ErrorTaxonomy = {
  ...
  blocked_signals: ["401_unauthorized", "403_forbidden", "cf_challenge"],
}

export function createClaudeWebReverseProvider(creds: ProviderCredentials): ProbeProvider {
  ...
  function buildHeaders(custom: Record<string, string>): Record<string, string> {
    ...
    if (auth.csrf_token) out["x-csrf-token"] = auth.csrf_token
    Object.assign(out, custom)
    return out
  }

  async function healthCheck(): Promise<HealthCheckResult> {
    const url = creds.health_check_url ?? `${creds.base_url.replace(/\/$/, "")}/api/organizations`
    try {
...
type ClaudeAuth = { session_cookie?: string; csrf_token?: string }

function parseAuth(json: string): ClaudeAuth {
  try {
    const parsed = JSON.parse(json) as Record<string, string>
    return {
      session_cookie: parsed.session_cookie ?? parsed.cookie,
...
function mapError(status: number | null, errMsg: string | null): ProbeError | undefined {
  ...
  if (status === 429) return { kind: "rate_limited", message: "claude-web 429", http_status: 429, retryable: true }
  if (status === 401 || status === 403) return { kind: "blocked", message: `claude-web ${status}`, http_status: status, retryable: false }
  if (status >= 500) return { kind: "http_error", message: `claude-web ${status}`, http_status: status, retryable: true }
  return undefined
}
