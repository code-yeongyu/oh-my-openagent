...
export function createOpencodeGoProvider(creds: ProviderCredentials): ProbeProvider {
  ...
  return {
    ...
    kind: "opencode_go",
    healthCheck,
    refreshCredentials: async (refreshType: string): Promise<CredentialRefreshResult> => ({ success: true, refresh_type: refreshType, message: "opencode-go uses static bearer tokens; no refresh" }),
...
function mapError(outcome: { ok: boolean; status: number | null; response_body: string | null; error_message?: string }): ProbeError | undefined {
  if (outcome.error_message) {
    ...
    if (m.includes("econn") || m.includes("refused") || m.includes("reset")) return { kind: "connection_refused", message: outcome.error_message, retryable: true }
    return { kind: "unknown", message: outcome.error_message, retryable: false }
  }
  if (outcome.status == null) return undefined
  if (outcome.status === 429) return { kind: "rate_limited", message: "opencode-go 429", http_status: 429, retryable: true }
  if (outcome.status === 401 || outcome.status === 403) return { kind: "blocked", message: `opencode-go ${outcome.status}`, http_status: outcome.status, retryable: false }
  if (outcome.status >= 500) return { kind: "http_error", message: `opencode-go ${outcome.status}`, http_status: outcome.status, retryable: true }
  return undefined
}
