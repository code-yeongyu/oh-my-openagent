...
export function createOllamaLocalProvider(creds: ProviderCredentials): ProbeProvider {
  ...
  async function healthCheck(): Promise<HealthCheckResult> {
    const url = creds.health_check_url ?? `${creds.base_url.replace(/\/$/, "")}/api/tags`
    try {
  ...
  return {
    ...
    kind: "ollama_local",
    healthCheck,
    refreshCredentials: async (refreshType: string): Promise<CredentialRefreshResult> => ({ success: true, refresh_type: refreshType, message: "ollama-local has no auth; refresh is a no-op" }),
...
function mapError(outcome: { ok: boolean; status: number | null; response_body: string | null; error_message?: string }): ProbeError | undefined {
  ...
  if (outcome.status === 404) return { kind: "http_error", message: `ollama-local 404 (model not pulled?)`, http_status: 404, retryable: false }
  if (outcome.status >= 500) return { kind: "http_error", message: `ollama-local ${outcome.status}`, http_status: outcome.status, retryable: true }
  return undefined
}
