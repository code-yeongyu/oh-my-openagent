...
const DEFAULT_REFERER = "https://idm.openrouter"
const DEFAULT_TITLE = "idm-probe-lab"

export function createOpenRouterProvider(creds: ProviderCredentials): ProbeProvider {
  const apiKey = parseApiKey(creds.auth_config)
  const referer = readAuthString(creds.auth_config, "http_referer") ?? DEFAULT_REFERER
  const xTitle = readAuthString(creds.auth_config, "x_title") ?? DEFAULT_TITLE
  ...
  async function healthCheck(): Promise<HealthCheckResult> {
    const url = creds.health_check_url ?? `${creds.base_url.replace(/\/$/, "")}/api/v1/models`
    try {
      const res = await fetch(url, { headers: buildHeaders({}) })
      return { ok: res.ok, status_code: res.status, message: res.ok ? "ok" : `health check returned ${res.status}`, checked_at: Math.floor(Date.now() / 1000) }
    } catch (err) {
  ...
  return {
    ...
    kind: "openrouter",
    healthCheck,
    refreshCredentials: async (refreshType: string): Promise<CredentialRefreshResult> => ({ success: true, refresh_type: refreshType, message: "openrouter uses static API keys; no refresh" }),
...
function parseApiKey(authConfigJson: string): string | null {
  try {
    ...
    return parsed.api_key ?? parsed.bearer_token ?? null
  } catch { return null }
}

function readAuthString(authConfigJson: string, key: string): string | null {
  try {
    const parsed = JSON.parse(authConfigJson) as Record<string, string>
...
function mapError(outcome: { ok: boolean; status: number | null; response_body: string | null; error_message?: string }): ProbeError | undefined {
  if (outcome.error_message) {
    const m = outcome.error_message.toLowerCase()
    if (m.includes("aborted") || m.includes("timeout")) return { kind: "timeout", message: outcome.error_message, retryable: true }
    return { kind: "unknown", message: outcome.error_message, retryable: false }
  }
  if (outcome.status == null) return undefined
  if (outcome.status === 429) return { kind: "rate_limited", message: "openrouter 429", http_status: 429, retryable: true }
  if (outcome.status === 401 || outcome.status === 403) return { kind: "blocked", message: `openrouter ${outcome.status}`, http_status: outcome.status, retryable: false }
  if (outcome.status >= 500) return { kind: "http_error", message: `openrouter ${outcome.status}`, http_status: outcome.status, retryable: true }
  return undefined
}
