...
const RESULT_PAYLOAD = "final answer payload"
...
describe("BackgroundManager.getTaskBySessionId", () => {
  ...
  test("#given a completed task removed from the live map #when looked up by bg_ id or session ID #then both resolve from the completed-task archive with the result payload retained", async () => {
    // given
    const task = await trackTrackedTask(manager)
    completeTask(task)
    ...
    expect(byBgId?.result).toBe(RESULT_PAYLOAD)
    expect(bySessionId?.id).toBe(TASK_ID)
    expect(bySessionId?.result).toBe(RESULT_PAYLOAD)
  })
