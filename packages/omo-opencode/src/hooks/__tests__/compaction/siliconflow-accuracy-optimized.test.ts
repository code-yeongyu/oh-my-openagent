/**
 * SiliconFlow Compaction Accuracy Tests - Optimized Version
...
describe.skipIf(shouldSkip)("SiliconFlow Compaction Accuracy Tests - Optimized", () => {
  let client: OpenAI
  let testProgress: ReturnType<typeof createTestProgress>
  ...
  beforeAll(() => {
    ...
    testProgress = createTestProgress(TestMode.STANDARD)
  })

  afterAll(() => {
    // 保存测试进度
    saveTestProgress(testProgress)
  })
...
答案：`
  ...
  describe("Short Session Accuracy (8 facts, 1 compaction)", () => {
    for (const model of TEST_MODELS) {
      it(`${model.name} - short session accuracy`, async () => {
        ...
        // 检查是否应该跳过（增量测试）
        if (shouldSkipTest(testProgress, model.id, "short-session")) {
          console.log(`Skipping ${model.name} (already completed)`)
          return
        }
        ...
        for (const fact of testFacts) {
          ...
          // 使用多层级准确率检查
          const checkResult = await checkAnswerAccuracy(client, model.id, answer, fact.expectedAnswer)
        ...
        // 标记为完成
        markTestCompleted(testProgress, model.id, "short-session")
  ...
  describe("Model Comparison Report", () => {
    it("generates comprehensive comparison with report", async () => {
      ...
      // 保存报告到文件
      const reportPath = `.cache/test-report-${Date.now()}.json`
      saveTestReport(report, reportPath)
