...
describe("checkInstallShadowing (#4451)", () => {
  ...
  test("#given two binaries with different versions on PATH #when running the check #then status is warn with a fix hint", async () => {
    ...
    try {
      ...
      expect(result.issues[0]?.fix).toContain("bun unlink")
      expect(result.issues[0]?.fix).toContain("bunx oh-my-openagent")
      expect(result.issues[0]?.fix).not.toContain("npm install -g")
      expect(result.issues[0]?.fix).not.toContain("bun add -g")
...
describe("scanFamilyInstalls + summarizeShadowing helpers (#4451)", () => {
  ...
  test("#given omo-agent-toolkit and an unrelated omo binary on PATH #when scanning #then only the family binary is found", async () => {
    const dir = mkdtempSync(join(tmpdir(), "omo-shadow-family-"))
    try {
      const toolkitPath = makeBinary(dir, "omo-agent-toolkit")
      makeBinary(dir, "omo")
      const entries = await scanFamilyInstalls({
        pathEntries: [dir],
        platform: "linux",
        resolveVersion: async () => "5.0.0",
      })
      expect(entries).toEqual([{ binary: "omo-agent-toolkit", path: toolkitPath, version: "5.0.0" }])
      expect(entries.some((entry) => entry.binary === "omo")).toBe(false)
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
