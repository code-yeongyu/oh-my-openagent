...
/** Binary basenames we consider part of the OmO family. Bare `omo` is excluded (unrelated npm package). */
const FAMILY_BINARIES = ["oh-my-opencode", "oh-my-openagent", "omo-agent-toolkit"] as const
...
export async function checkInstallShadowing(deps?: Partial<ScanDeps>): Promise<CheckResult> {
  ...
  const name = CHECK_NAMES[CHECK_IDS.INSTALL_SHADOWING]
  ...
  const issue: DoctorIssue = {
    ...
    fix:
      "Remove stale PATH binaries (for example `npm uninstall -g oh-my-opencode` or `bun unlink`) so a single version remains. " +
      "Prefer `bunx oh-my-openagent` rather than adding another global install.",
