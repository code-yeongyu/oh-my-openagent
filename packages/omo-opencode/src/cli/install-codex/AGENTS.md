...
| `link-cached-plugin-agents.ts` | Discovers bundled agent TOMLs under `components/*/agents/`, copies to `~/.codex/agents/`, preserves user-modified agent TOMLs via `~/.codex/.omo/native-agents.json` hashes, writes `.installed-agents.json` manifest |
