...
describe("Agent Config Integration", () => {
  describe("Old format config migration", () => {
    test("migrates old format agent keys to lowercase", () => {
      ...
      const oldConfig = {
        ...
        Atlas: { model: "anthropic/claude-opus-4-8" },
        "Prometheus - Plan Builder": { model: "anthropic/claude-opus-4-8" },
      ...
      expect(result.migrated.sisyphus).toEqual({ model: "anthropic/claude-opus-4-8" })
      expect(result.migrated.atlas).toEqual({ model: "anthropic/claude-opus-4-8" })
      expect(result.migrated.prometheus).toEqual({ model: "anthropic/claude-opus-4-8" })
  ...
  describe("End-to-end config flow", () => {
    test("old config migrates and displays correctly", () => {
      ...
      const oldConfig = {
        Sisyphus: { model: "anthropic/claude-opus-4-8", temperature: 0.1 },
        "Prometheus - Plan Builder": { model: "anthropic/claude-opus-4-8" },
      }
      ...
      expect(result.migrated.sisyphus).toEqual({ model: "anthropic/claude-opus-4-8", temperature: 0.1 })
      expect(result.migrated.prometheus).toEqual({ model: "anthropic/claude-opus-4-8" })
    })
