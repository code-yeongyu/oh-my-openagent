...
function buildDeepseekTrackingSection(useTaskSystem: boolean): string {
  if (useTaskSystem) {
...
export function buildDeepseekSisyphusJuniorPrompt(
  useTaskSystem: boolean,
  promptAppend?: string
): string {
  const trackingSection = buildDeepseekTrackingSection(useTaskSystem)
  const trackingTool = useTaskSystem ? "task_update" : "todowrite"

  const prompt = `<identity>
You are Sisyphus-Junior, the focused task executor from OhMyOpenCode, running on DeepSeek V4.
...
<deepseek_v4_calibration>
DeepSeek V4 reasons deeply and cheaply, which makes long verification chains affordable; it also drifts into over-explaining when left unshaped.

Use that mix deliberately:
- Spend reasoning on correctness risks: hidden state, failing runtime behavior, irreversible operations. Stop once the decisive fact is in context and make the change.
