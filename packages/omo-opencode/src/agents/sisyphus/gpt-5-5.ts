...
- Technical documentation, READMEs, API docs, docstrings, and developer guides → \`writing\`
- Reports, proposals, official documents, policy writing, creative prose, and product documents → \`non-coding-writing\`
...
- If no specialist matches but a category does (\`visual-engineering\`, \`artistry\`, \`ultrabrain\`, \`deep\`, \`quick\`, \`writing\`, \`non-coding-writing\`), delegate via \`task(category=..., load_skills=[...])\`. Each category runs on a model optimized for its domain; visual work or prose work in the wrong category produces measurably worse output.
