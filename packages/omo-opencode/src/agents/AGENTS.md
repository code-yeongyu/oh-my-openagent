...
|-------|---------------|------|------|--------------------------|---------|
| **Sisyphus** | claude-opus-4-8 max | (model default) | primary | kimi-k3 → gpt-5.6-sol medium → glm-5 → big-pickle | Main orchestrator, plans + delegates; `thinking: { type: "enabled", budgetTokens: 32000 }` |
...
| **Oracle** | gpt-5.6-sol xhigh (high on Copilot) | 0.1 | subagent | gemini-3.1-pro high → claude-opus-4-8 max → glm-5.2 | Read-only consultation |
...
| **Metis** | claude-sonnet-4-6 | **0.3** | subagent | claude-opus-4-8 max → gpt-5.6-sol medium → glm-5.2 → kimi-k3 | Pre-planning consultant |
| **Momus** | gpt-5.6-terra high | 0.1 | subagent | gpt-5.6-sol xhigh (high on Copilot) → claude-opus-4-8 max → gemini-3.1-pro high → glm-5.2 | Plan reviewer |
| **Atlas** | claude-sonnet-4-6 | 0.1 | primary | kimi-k3 → gpt-5.6-sol medium → minimax-m3 → MiniMax-M3 → minimax-m2.7 | Todo-list orchestrator |
| **Prometheus** | claude-opus-4-8 max | (override-only) | primary | gpt-5.6-sol high → glm-5.2 → gemini-3.1-pro | Strategic planner (interview); built via `buildPrometheusAgentConfig` (not in `agentSources`) |
