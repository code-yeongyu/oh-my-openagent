...
const delegateTaskArgsSchema = {
  ...
  run_in_background: tool.schema
    .boolean()
    .optional()
    .describe("Optional; defaults to false (sync). true=async (returns background task ID `bg_...` for background_output), false=sync (waits). Prefer true for independent parallel work that does not gate the caller's next step; use false when the caller needs the result before continuing. Give concurrent executors that write files disjoint ownership."),
