...
export async function removeOwnedWorktreeDirectory(
  ...
  try {
    let metadata
    try {
      metadata = await lstat(ownedWorktreeRoot)
    } catch (error) {
      if (!hasErrorCode(error, "ENOENT")) throw error
      try {
        await lstat(getOwnerMarkerPath(ownedWorktreeRoot))
      } catch (markerError) {
        if (hasErrorCode(markerError, "ENOENT")) return { removed: false }
        throw markerError
      }
      return { removed: false, error: "owned worktree root is missing while its owner marker remains" }
    }
...
export async function removeOwnedWorktreeDirectories(
  ...
  for (const ownership of ownershipRecords) {
    ...
    if (!root || !ownership.worktreeOwnershipToken || !ownership.worktreeCanonicalPath) continue
    if (root && seenRoots.has(root)) continue
    if (root) seenRoots.add(root)
    const result = await removeOwnedWorktreeDirectory(ownership)
    if (result.removed && root) removedWorktrees.push(root)
    if (result.error) errors.push(`worktree ${root ?? "<unknown>"}: ${result.error}`)
  }
