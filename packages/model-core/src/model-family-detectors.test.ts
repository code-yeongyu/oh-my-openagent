...
describe("model family detectors", () => {
  ...
  test("#given Kimi K3 model ids #then detects K3 only, not K2.x", () => {
    ...
    expect(isKimiK3Model("kimi-for-coding/k3-256k")).toBe(true)
    expect(isKimiK3Model("k3-256k")).toBe(true)
    expect(isKimiK3Model("KIMI-FOR-CODING/K3-256K")).toBe(true)
    expect(isKimiK3Model("not-k3-256k")).toBe(false)
    expect(isKimiK3Model("somek3-256k")).toBe(false)
    expect(isKimiK3Model("k3-256kb")).toBe(false)
    expect(isKimiK3Model("k3-context-256k")).toBe(false)
    expect(isKimiK3Model("gpt-k3-256k")).toBe(false)
    expect(isKimiK3Model("k2-256k")).toBe(false)
