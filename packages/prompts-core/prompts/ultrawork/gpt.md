...
- Use CodeGraph first when CodeGraph tools are available for how/where/what/flow questions and before edits. Use the exact CodeGraph tool name shown in your current tool list: OpenCode commonly exposes `codegraph_codegraph_explore`, Codex MCP form may expose `mcp__codegraph__codegraph_explore`, and bare `codegraph_explore` is valid only when that exact bare name is exposed. If absent or inactive/cold-start unavailable, continue with Grep/Read/LSP and the ast-grep skill.
...
|-------|-------|-------|---------|
| **Direct** | CodeGraph exact exposed explore tool (for example `codegraph_codegraph_explore`, `mcp__codegraph__codegraph_explore`, or bare `codegraph_explore` only if exposed), Grep, Read, LSP, ast-grep skill (`sg`) | Instant | Quick wins, known locations |
