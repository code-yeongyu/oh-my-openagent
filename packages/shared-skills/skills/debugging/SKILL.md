...
|---|---|---|
| **Browser Playwright / Playwright CLI** | Any browser-served web UI bug. In Codex, use `browser:control-in-app-browser` and its `tab.playwright` capability first; use standalone Playwright only as a fallback when the Browser tool is unavailable or insufficient. **For Phase 8 QA of any browser product, you MUST drive a real browser — not curl, not imagination.** | 📖 **[references/tools/playwright-cli.md](references/tools/playwright-cli.md)** |
...
**Failing to use these tools in their domain is a process failure, not a stylistic choice.** If the bug is in a browser and you did Phase 8 without a Playwright-driven browser surface, you are doing it wrong. In Codex, that surface is the Browser tool's `tab.playwright` capability first and standalone Playwright only as fallback. If the bug is in a stripped binary and you read hex with `xxd`, you are doing it wrong. The references tell you how. Read them.
