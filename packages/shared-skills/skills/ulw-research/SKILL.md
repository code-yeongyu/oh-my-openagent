...
Assembly worker: `task(category="deep", load_skills=["frontend", "visual-qa", "imagegen", "ulw-loop"], run_in_background=true, ...)`. Before writing, read the listed design and visualization skills and apply them; the report is a designed artifact, not a text dump. Let `frontend` own design-reference routing, including its configured local Open Design extension boundary when built-in coverage is insufficient or the user explicitly requests Open Design. Add a separately installed data-visualization skill only when it is actually available. Run HTML/PDF output through the ULW loop with frontend and visual-qa, then repair until the reviewer approves. Structure: executive summary -> key findings by theme -> detailed analysis with short attributed quotes, charts, Mermaid graphs, generated visuals, SHA-pinned permalinks, and verification results -> comparative analysis when options compete -> numbered sources with access dates -> methodology appendix. Every claim cites `[Source N]`.
