...
- Skip the ask when a default output is on record: an explicit user-set default in project memory or config, or a prior journal answer reused silently; a per-run explicit choice wins; record a newly stated default.
