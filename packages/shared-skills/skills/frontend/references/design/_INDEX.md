...
| File | Purpose | Load when |
|---|---|---|
| `taste-skill.md` | v2 brief-inference workflow for landing pages, portfolios, and marketing redesigns: one-line Design Read, 3 dials (DESIGN_VARIANCE, MOTION_INTENSITY, VISUAL_DENSITY) inferred from the brief, a Brief → Design System Map (official packages for system-shaped briefs), and a hard final pre-flight check. Scopes OUT dashboards, data tables, and dense product UI. | Marketing surface with no named brand and no wow demand. For operational briefs, load ONLY its Brief → Design System Map section to pick an official design system as the token source. |
