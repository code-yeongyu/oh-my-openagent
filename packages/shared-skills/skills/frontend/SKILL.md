...
|---|---|
| ANY UI implementation, styling, redesign, mockup, or visual decision | `references/design/README.md` FIRST. It enforces the Design System Gate (a `DESIGN.md` must exist before any component is written), numeric reuse governance, surface label routing, and the React Dev Tooling Gate (react-grab / react-scan / react-doctor installed by default) before routing to the taste and brand references below. |
