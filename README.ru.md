...
| 🧩   | **Codex CLI Light Edition**                              | Light    | 5 компонентов omo, портированных в OpenAI Codex CLI (rules, comment-checker, LSP, ultrawork, ulw-loop). Установка: `bunx --package oh-my-openagent omo install --platform=codex`.                                                                          |
...
> **Editions, легенда.** **Ultimate** = только OpenCode (`bunx --package oh-my-openagent omo install`). **Light** = только Codex CLI (`bunx --package oh-my-openagent omo install --platform=codex`). **Both** = поставляется в обеих редакциях, часто с немного отличающейся реализацией.
