...
- **explore** (`opencode/grok-code` または `google/gemini-3-flash`): 高速なコードベース探索、ファイルパターンマッチング。Antigravity 認証が設定されている場合は Gemini 3 Flash を使用し、それ以外は Grok を使います—無料で、極めて高速で、ファイル探索タスクには十分です。Claude Code からインスピレーションを得ました。
