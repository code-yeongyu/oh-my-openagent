...
| **lsp_goto_definition** | Jump to symbol definition (replaced by OpenCode's unified `lsp` tool when `lsp.useOpenCodeExperimental: true`) |
| **lsp_find_references** | Find all usages across workspace (replaced by OpenCode's unified `lsp` tool when `lsp.useOpenCodeExperimental: true`) |
| **lsp_symbols** | Get file outline or workspace symbol search |

**Note**: When `lsp.useOpenCodeExperimental: true`, `lsp_goto_definition` and `lsp_find_references` are replaced by OpenCode's built-in unified LSP tool. See the [migration guide](migration/opencode-lsp.md) for details.
