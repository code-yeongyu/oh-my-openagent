...
> **Warning**: If you are on npm stable 4.19.4, keep the generated working configuration and treat `doctor` warnings about its `variant` / `fallback_models` settings as non-fatal; 4.19.4 accepts those settings as generated. When the reasoning-unification migration runs, it can rewrite those agent chains to the canonical `agents.*.models` form, which the 4.19.4 plugin validator rejects. Back up `~/.omo/omo.jsonc` and do not run migration commands on 4.19.4, including `--dry-run`: a pending `~/.omo/.migration-journal.json` can be resumed before dry-run handling. Migrated chain validation support is merged on `dev` via [PR #6602](https://github.com/code-yeongyu/oh-my-openagent/pull/6602); broader runtime fallback behavior remains tracked by [issue #6644](https://github.com/code-yeongyu/oh-my-openagent/issues/6644) and [PR #6668](https://github.com/code-yeongyu/oh-my-openagent/pull/6668).
