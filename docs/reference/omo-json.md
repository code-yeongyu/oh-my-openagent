...
| `dag` | object | unset |
| `transcript_retention` | object | unset (retention off) |
...
`task.transcript_retention` is an optional block enabling bounded, opt-in retention of selected task transcripts (`schema/task.ts`): `enabled` (`false`), `categories` (`["deep"]`), `ttl_ms` (`604800000`, 7 days, mirroring `task.dag.retention_days`), `max_bytes` (`262144`), `success_sample_denominator` (`4`), `metadata_only_paths` (`[]`).

When enabled, the TTL sweep archives a selected terminal task's visible transcript (the original prompt, assistant responses, tool execution markers, and error diagnostics that the engine already records - never hidden provider reasoning) as TWO artifacts under `<agent dir>/sessions/--omo-senpi-task-archive--/`: a finder-discoverable session JSONL manifest holding the task's metadata (parent task/session ids, requested/resolved model, category, cwd, start/end/outcome, usage, final response, retention reason) plus an integrity pointer, and a gzip-compressed `<same name>.jsonl.gz` holding the size-bounded (head+tail within `max_bytes`) session-shaped transcript itself. Compression is deterministic (zeroed gzip mtime), so re-archiving the same task is byte-identical; the manifest pointer carries the sidecar's sha256 and byte sizes. All files are mode 0600 (dir 0700) and expire after `ttl_ms`. Well-known credential shapes (api keys, bearer/JWT tokens, private key blocks) are redacted from every archived text surface before anything is written. A task is selected when it spawns with the task tool's `retain_transcript: true` (full archive) or `retain_transcript: "metadata"` (manifest only - never transcript content; the mode for protected work such as clinical content), or, without an explicit mark, when its category is in `categories`; `retain_transcript: false` always excludes entirely. Any selected task whose spawn cwd lies inside a `metadata_only_paths` entry is downgraded to a metadata-only archive, so a protected tree can never contribute transcript content even when a category rule selects it. Failed/cancelled/interrupted/lost selected tasks are always archived; successful category-rule selections only when `sha256(task id) % success_sample_denominator` is 0, so the archive stays bounded (explicit marks are never sampled away). Nothing is retained until you set `enabled: true`, and archives live only on your machine.
