...
| **agent-audit**    | "audit this agent", "why is the wrapper worse", "tool discipline", "memory contamination" | Structured runtime audit skill for diagnosing wrapper regression, stale memory contamination, hidden repair layers, rendering mutation, and tool-discipline failures. Produces typed audit artifacts, severity-ranked findings, contamination paths, and an ordered fix plan instead of a freeform review. |
