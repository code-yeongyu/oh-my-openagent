...
When running inside cmux, start OMO with `cmux omo-agent-toolkit`. OMO routes pane commands through cmux's tmux-compatible interface and does not require a native `tmux` binary on `PATH`. Because that interface cannot safely receive tmux `-e` bindings, OMO creates a cmux pane only when the required pane environment is safely omissible; authenticated listener variables therefore fail closed instead of being inherited or exposed.
