...
Repository beta releases are dispatched with `/publish <explicit-semver>`, for example `/publish 5.0.0-beta.9`. The command sends that exact value through the workflow's `version` input, records the returned workflow run ID, and follows only that run. Release notes compare a beta against the preceding beta in the same channel. A semver prerelease is created as a GitHub prerelease with `--latest=false`; stable releases are full releases, and [`script/release-latest-flag.ts`](../../script/release-latest-flag.ts) decides their **Latest** badge from the highest already published semver (`Bun.semver` ordering, non-semver tags such as `_pr-attachments` ignored). This keeps beta, rc, and alpha channels out of the stable Latest pointer while preventing an older-line stable hotfix dispatched later from stealing the badge.
