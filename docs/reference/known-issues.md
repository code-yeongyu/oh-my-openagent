...
## #6644 - Stable 4.19.4 migration can rewrite agent chains its plugin validator rejects
...
- **Symptom**: Stable 4.19.4 generates and accepts those `variant` / `fallback_models` settings, so the configuration works on that release even though `doctor` reports non-fatal deprecation warnings for them. When the reasoning-unification migration runs, it can rewrite those agent chains to the canonical `agents.*.models` form, which the 4.19.4 plugin validator rejects, so the migrated configuration fails validation on that release.
- **Workaround**: On stable 4.19.4, keep the generated working configuration and treat the `doctor` warnings as non-fatal. Do not run migration commands manually, including `--dry-run`: a pending `~/.omo/.migration-journal.json` can be resumed before dry-run handling. Startup and install also invoke migration, so back up `~/.omo/omo.jsonc` before restarting or reinstalling and restore the generated working configuration if an automatic run changes it.
- **Status**: Open. Migrated chain validation support is merged on `dev` and published under npm's `beta` tag by https://github.com/code-yeongyu/oh-my-openagent/pull/6602. Broader canonical runtime fallback behavior remains tracked by https://github.com/code-yeongyu/oh-my-openagent/issues/6644 and https://github.com/code-yeongyu/oh-my-openagent/pull/6668. npm `latest` (4.19.4) remains affected pending a stable release containing the fixes.
