...
- **Workaround**: On stable 4.19.4, keep the generated working configuration and treat the `doctor` warnings as non-fatal. Back up `~/.omo/omo.jsonc` and do not run migration commands, including `--dry-run`: a pending `~/.omo/.migration-journal.json` can be resumed before dry-run handling.
