...
| Model            | Strengths                                                                                                                                             |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **GPT-5.5**      | High intelligence, strategic reasoning. Default for Hephaestus, Oracle, Momus, `deep`, and `ultrabrain`. Uses xhigh variant for Momus and ultrabrain. |
| **GPT-5.4**      | Compatibility-era GPT family. Existing configs migrate to GPT-5.5, while Mini/Fast/Nano variants remain active utility models.                        |
...
### Google Gemini provider IDs

Runtime chains use `gemini-3.1-pro` as the cross-provider target. Google and Vercel catalogs may expose the public model as `gemini-3.1-pro-preview`; OpenCode's model resolver can fuzzy-match the runtime target to the provider-specific preview ID when that is what the connected provider advertises.
