...
| Agent                 | Role               | Fallback Chain                                 | Notes                                                 |
| --------------------- | ------------------ | ---------------------------------------------- | ----------------------------------------------------- |
| **Explore**           | Fast codebase grep | Grok Code Fast → opencode-go/minimax-m2.7 → MiniMax Free → Haiku → GPT-5-Nano | Speed is everything. Fire 10 in parallel.             |
| **Librarian**         | Docs/code search   | opencode-go/minimax-m2.7 → MiniMax Free → Haiku → GPT-5-Nano                  | Doc retrieval doesn't need deep reasoning.            |
...
| **MiniMax M2.7**     | Fast and smart. Good for utility tasks and search/retrieval.                                                 |
...
| **opencode-go/minimax-m2.7** | Ultra-cheap, fast responses. Used by Librarian, Explore for utility work.                          |
...
You may see model names like `kimi-k2.5-free`, `minimax-m2.7-free`, or `big-pickle` (GLM 4.6) in the source code or logs. These are free-tier versions of the same model families, served through the OpenCode Zen provider. They exist as lower-priority entries in fallback chains.
