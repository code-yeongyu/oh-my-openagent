...
Set `tmux_visualization: true`. Native tmux usage requires running inside tmux with `tmux` on `PATH`. For cmux, start OMO with `cmux omo-agent-toolkit`; its tmux-compatible interface is used without requiring a native tmux binary. Both modes require an OpenCode HTTP listener started with `opencode --port <port>`. Reachability alone does not establish readiness. The `/global/health` probe reuses `OPENCODE_SERVER_PASSWORD` and optional `OPENCODE_SERVER_USERNAME` only when OpenCode reports the current listener URL or `OPENCODE_PORT` identifies it explicitly; the username defaults to `opencode`. If OMO cannot verify that an address belongs to the current listener, such as a default address or one left by an older pane, it probes without credentials. Redirects and unsuccessful responses do not establish readiness. Each pane creation or respawn revalidates readiness and uses the exact authentication snapshot that passed its probe. Team layout obtains one snapshot for each member pane; that pane's creation and setup reuse it. Layout selection and resizing do not start a process and do not perform a separate health probe. Simultaneous checks share one request only when the normalized listener, authentication settings, and request transport match; credentials are not stored.

Native tmux panes targeting that verified listener receive the current server authentication variables through `split-window -e`; the installed tmux must support that flag. Panes targeting another address clear those variables in the child process without changing tmux's global environment. cmux panes are created only when the required environment is safely omissible: an authenticated listener fails closed because cmux cannot safely receive the tmux `-e` bindings. Existing panes are not changed retroactively. Failures remain isolated: if the backend is unavailable, the listener is not ready, or the pane environment cannot be applied safely, background agents and teams continue while only pane visualization is skipped.
...
`team_delete` closes panes only after matching their recorded backend target and team ownership marker. Legacy layout state without that identity fails closed and can leave panes for manual cleanup. Per-member shutdown closes just that pane and rebalances the remaining layout.
