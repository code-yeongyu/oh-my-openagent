...
- **Codex config edits:** `~/.codex/config.toml` gained `[features] plugins = true`, `[features] plugin_hooks = true`, `[features.multi_agent_v2] max_concurrent_threads_per_session = 1000` by default, `[marketplaces.sisyphuslabs]` pointing at `~/.codex/plugins/cache/sisyphuslabs`, `[plugins."omo@sisyphuslabs"]`, plugin MCP policy blocks, SHA256-pinned `[hooks.state."omo@sisyphuslabs:..."]` entries, and optionally autonomous permission settings if accepted. If the installer cannot resolve a CodeGraph-compatible Node runtime, it writes the `codegraph` MCP policy as disabled while leaving `omo@sisyphuslabs` enabled.

To use a lower managed subagent cap, set `LAZYCODEX_SUBAGENT_THREAD_LIMIT` to an integer from `1` through `1000` before installing and when launching Codex. The installer and the `SessionStart` migration both apply it. The OMO-prefixed name, `OMO_CODEX_SUBAGENT_THREAD_LIMIT`, is accepted as a compatibility alias; the `LAZYCODEX_` variable wins when both are set, and invalid values use the default `1000`.
