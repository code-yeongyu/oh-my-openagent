...
|   🛠️   | **LSP + AST-Grep**                                       | Both     | 워크스페이스 리네임, 빌드 전 진단, AST 기반 리라이트. Ultimate는 내장 도구로, Light는 plugin-scoped `lsp` / `ast_grep` MCP로 제공합니다.                                                     |
|   🧠   | **Background Agents**                                    | Ultimate | 전문가 5명 이상을 동시에 발사. 컨텍스트는 가볍게. 결과는 준비되면 도착.                                                                                                                                          |
|   📚   | **Built-in MCPs**                                        | Both     | Ultimate는 Exa(웹 검색), Context7(공식 문서), Grep.app(GitHub 검색)를 런타임에 주입합니다. Light는 plugin-scoped MCP로 `ast_grep`, `grep_app`, `context7`, `git_bash`, `lsp`를 제공합니다.                                                                                                                   |
