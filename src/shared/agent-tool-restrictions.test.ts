...
describe("agent-tool-restrictions", () => {
  ...
  test("#given Athena-Junior council member name #when getAgentToolRestrictions #then returns solo runtime restrictions", () => {
    ...
    expect(restrictions.read).toBe(true)
    expect(restrictions.grep).toBe(true)
    expect(restrictions.glob).toBe(true)
    expect(restrictions.lsp_goto_definition).toBe(true)
    expect(restrictions.lsp_find_references).toBe(true)
    expect(restrictions.lsp_symbols).toBe(true)
    expect(restrictions.lsp_diagnostics).toBe(true)
    expect(restrictions.ast_grep_search).toBe(true)
    expect(restrictions.call_omo_agent).toBe(false)
