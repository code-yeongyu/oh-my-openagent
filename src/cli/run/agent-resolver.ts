...
const isAgentDisabled = (agentConfigKey: string, config: MatrixxConfig): boolean => {
  const lowered = agentConfigKey.toLowerCase()
  if (lowered === DEFAULT_AGENT && config.morpheus_agent?.disabled === true) {
    return true
  }
...
const pickFallbackAgent = (config: MatrixxConfig): CoreAgentKey => {
