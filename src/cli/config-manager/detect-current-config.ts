...
function detectProvidersFromOmoConfig(): {
  ...
  try {
    ...
    const hasOpenRouter = configStr.includes('"openrouter/')

    return { hasOpenAI, hasOpencodeZen, hasZaiCodingPlan, hasKimiForCoding, hasOpencodeGo, hasVercelAiGateway, hasOpenRouter }
  } catch {
    return {
      hasOpenAI: true,
      hasOpencodeZen: true,
...
export function detectCurrentConfig(): DetectedConfig {
  ...
  const { hasOpenAI, hasOpencodeZen, hasZaiCodingPlan, hasKimiForCoding, hasOpencodeGo, hasVercelAiGateway, hasOpenRouter } = detectProvidersFromOmoConfig()
