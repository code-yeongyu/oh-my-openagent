...
// NOTE(non-interactive-athena, 2026-02-28):
// We intentionally removed non-interactive mode instructions from Athena's prompt while redesigning
// interactive/background behavior for council orchestration. Preserve prior intent here for reference:
// - If Question tool is unavailable, auto-select all registered council members with delegation mode.
// - Launch members with write_output_to_file=true, track via background_wait, collect via council_finalize.
// - After synthesis, auto-select next action by question type (ACTIONABLE -> Atlas, INFORMATIONAL -> end).
// - Never attempt Question tool calls when unavailable.
