...
describe("createStallInjectorHook", () => {
  test("#given task stalled 5m30s #when chat.message #then injects warning alert (5min default threshold)", async () => {
    // given
    const now = Date.now()
    const tasks = [
      makeTask({
  ...
  test("#given task stalled 10m30s #when chat.message #then injects critical alert (10min default threshold)", async () => {
    // given
    const now = Date.now()
    const tasks = [
      makeTask({
  ...
  test("#given task already alerted at same level #when second chat.message #then suppresses duplicate alert", async () => {
    // given
    const now = Date.now()
    const tasks = [
      makeTask({
  ...
  test("#given 3 stalled tasks #when chat.message #then batches into single alert with per-task IDs", async () => {
    // given
    const now = Date.now()
  ...
  test("#given warning then critical escalation on same task #when levels differ #then injects critical alert bypassing same-level rate limit", async () => {
    const now = Date.now()
  ...
  test("#given explicit 5m/10m stall thresholds via config #when chat.message on stalled task #then injects status message with guidance", async () => {
    ...
    const tasks = [
      makeTask({
        ...
        progress: makeProgress({
          lastTool: "test",
          lastUpdate: new Date(now - 305_000),
        }),
      }),
    ]
  ...
  test("#given stalled team-mode task with teamRunId #when chat.message #then does not inject", async () => {
    const now = Date.now()
    const tasks = [
      makeTask({
        id: "team-task-1",
