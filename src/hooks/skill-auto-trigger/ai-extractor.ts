...
export async function extractTriggersWithAI(
	...
	if (createResult.error || !createResult.data?.id) {
		throw new Error(
			`Failed to create extraction session: ${createResult.error}`,
		);
	}
	...
	try {
		...
		try {
			...
			if (!extractionModel) {
				...
				log(
					"[skill-auto-trigger] Prompt failed with model override, retrying without model",
					{
						sessionID,
						error: errorMessage,
					},
				...
				try {
					...
					if (fallbackMessage.includes("Unexpected EOF")) {
						log(
							"[skill-auto-trigger] Prompt error after fallback, continuing to poll",
							{
								sessionID,
...
export async function triggerBackgroundExtraction(
	...
	const newCacheSkills: Record<string, CachedSkillTrigger> = {
		...currentCache.skills,
	};
	...
	for (const batch of batches) {
		try {
			...
			log(
				`[${HOOK_NAME}] Extracted triggers for batch of ${batch.length} skills`,
			);
		} catch (err) {
	...
	log(
		`[${HOOK_NAME}] Background extraction complete, saved ${Object.keys(newCacheSkills).length} skills to cache`,
	);
