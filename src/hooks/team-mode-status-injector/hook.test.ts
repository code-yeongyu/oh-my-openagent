...
describe("createTeamModeStatusInjector", () => {
  ...
  it("injects team mode status when a custom team mode pattern matches", async () => {
    // given
    const hook = createTeamModeStatusInjector(
      TeamModeConfigSchema.parse({ enabled: true }),
      { modes: { team: { pattern_append: "|customteamword" } } },
    );
    const output = createOutput("session-team-mode", "customteamword");
    ...
    // then
    expect(output.messages).toHaveLength(2);
    expect(output.messages[0]?.parts[0]?.text).toEqual(
      expect.stringContaining('<team_mode_status enabled="true">'),
    );
  });

  it("does not inject when team keywords are disabled even with a custom team mode pattern", async () => {
    // given
    const hook = createTeamModeStatusInjector(
      TeamModeConfigSchema.parse({ enabled: true }),
      {
    ...
    // then
    expect(output.messages).toHaveLength(1);
    expect(output.messages[0]?.parts[0]?.text).toBe("customteamword");
  });
});
