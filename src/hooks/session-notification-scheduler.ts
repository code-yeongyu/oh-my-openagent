...
type ShouldPlaySound = (sessionID: string) => boolean
...
export function createIdleNotificationScheduler(options: {
  ...
  async function executeNotification(sessionID: string, version: number): Promise<void> {
    ...
    try {
      ...
      const shouldPlaySound = options.shouldPlaySound?.(sessionID) ?? true
      if (shouldPlaySound && options.config.playSound && options.config.soundPath) {
