...
function suppressComboStandalones(
  ...
  return detected.filter(
    (k) => k.type !== "ultrawork" && k.type !== "hyperplan",
  );
}
    ...
    "chat.message": async (
      ...
      if (isPlannerAgent(currentAgent)) {
        ...
        if (preFilterCount > detectedKeywords.length) {
          log(
            `[keyword-detector] Filtered ultrawork/hyperplan keywords for planner agent`,
      ...
      const hasHyperplanUltrawork = detectedKeywords.some(
        (k) => k.type === "hyperplan-ultrawork",
      );
