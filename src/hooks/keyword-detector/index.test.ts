  ...
  test("should tell analyze-mode agents to evaluate skills before delegating", async () => {
    ...
    const output = {
      ...
      parts: [
        {
          type: "text",
          text: "investigate why subagents miss recovery skills",
        },
      ],
    };
    ...
    expect(textPart).toBeDefined();
    expect(textPart!.text).toContain(
      "Evaluate available skills before dispatch",
    );
...
describe("keyword-detector agent-specific ultrawork messages", () => {
  ...
  test("should skip ultrawork for prometheus but inject for sisyphus", async () => {
    ...
    const sisyphusTextPart = sisyphusOutput.parts.find(
      (p) => p.type === "text",
    );
...
describe("keyword-detector disabled_keywords config", () => {
  ...
  test("should apply pattern_append from hook config", async () => {
    // given - keyword detector with custom pattern_append
    const sessionID = "pattern-append-session";
    getMainSessionSpy = spyOn(sessionState, "getMainSessionID").mockReturnValue(
      sessionID,
    );
    const hook = createKeywordDetectorHook(
      ...
      undefined,
      undefined,
      { modes: { search: { pattern_append: "|customword" } } },
    );
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "customword for the bug" }],
    };

    // when - custom keyword fires
    await hook["chat.message"]({ sessionID }, output);

    // then - search-mode should inject
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
    expect(textPart!.text).toContain("[search-mode]");
  });

  test("should apply message_append from hook config", async () => {
    // given - keyword detector with custom message_append
    const sessionID = "message-append-session";
    getMainSessionSpy = spyOn(sessionState, "getMainSessionID").mockReturnValue(
      sessionID,
    );
    const hook = createKeywordDetectorHook(
    ...
    // when - search keyword fires
    await hook["chat.message"]({ sessionID }, output);

    // then - search-mode should inject with custom note
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
    expect(textPart!.text).toContain("[search-mode]");
    expect(textPart!.text).toContain("Custom search note.");
  });

  test("should apply message override from hook config", async () => {
    // given - keyword detector with custom message override
    const sessionID = "message-override-session";
    getMainSessionSpy = spyOn(sessionState, "getMainSessionID").mockReturnValue(
      sessionID,
    );
    const hook = createKeywordDetectorHook(
      ...
      undefined,
      undefined,
      { modes: { search: { message: "[custom-search] Custom message." } } },
    );
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "search for the bug" }],
    ...
    // then - custom message should replace default
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
    expect(textPart!.text).toContain("[custom-search] Custom message.");
    expect(textPart!.text).not.toContain("[search-mode]");
  });
});
