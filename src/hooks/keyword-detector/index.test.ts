...
describe("keyword-detector session filtering", () => {
  ...
  test("should skip non-ultrawork keywords in non-main session (using mainSessionID check)", async () => {
    ...
    const skipLog = logCalls.find((c) =>
      c.msg.includes(
        "Skipping non-ultrawork/ultrapower keywords in non-main session",
      ),
    );
...
describe("keyword-detector agent-specific ultrawork messages", () => {
  ...
  test("should use planner-specific ultrawork message when agent is prometheus", async () => {
    ...
    const pending = collector.getPending(sessionID);
    const ultraworkEntry = pending.entries.find(
      (e) => e.id === "keyword-ultrawork",
    );
    expect(ultraworkEntry).toBeDefined();
    expect(ultraworkEntry!.content).toContain(
      "YOU ARE A PLANNER, NOT AN IMPLEMENTER",
    );
    expect(ultraworkEntry!.content).not.toContain(
  ...
  test("should switch from planner to normal message when agent changes", async () => {
    ...
    const prometheusEntry = prometheusPending.entries.find(
      (e) => e.id === "keyword-ultrawork",
    );
    expect(prometheusEntry!.content).toContain(
      "YOU ARE A PLANNER, NOT AN IMPLEMENTER",
    );

    const sisyphusPending = collector.getPending(sisyphusSessionID);
    const sisyphusEntry = sisyphusPending.entries.find(
      (e) => e.id === "keyword-ultrawork",
    );
    expect(sisyphusEntry!.content).toContain(
      "YOU MUST LEVERAGE ALL AVAILABLE AGENTS",
    );
  });
...
describe("keyword-detector ultrapower mode", () => {
  let logCalls: Array<{ msg: string; data?: unknown }>;
  let logSpy: ReturnType<typeof spyOn>;
  ...
  test("should register ultrapower keyword to ContextCollector", async () => {
    // #given - a fresh ContextCollector and keyword-detector hook
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "ultrapower-test-session";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrapower do something" }],
    };

    // #when - keyword detection runs
    await hook["chat.message"]({ sessionID }, output);

    // #then - ultrapower context should be registered in collector
    expect(collector.hasPending(sessionID)).toBe(true);
    const pending = collector.getPending(sessionID);
    expect(pending.entries.some((e) => e.id === "keyword-ultrapower")).toBe(
      true,
    );
  });

  test("should trigger ultrapower on 'ulo' keyword", async () => {
    // #given - text contains 'ulo' keyword
    const toastCalls: string[] = [];
    const hook = createKeywordDetectorHook(
      createMockPluginInput({ toastCalls }),
    );
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ulo implement this feature" }],
    };

    // #when - message with 'ulo' is processed
    await hook["chat.message"]({ sessionID: "any-session" }, output);

    // #then - ultrapower should be triggered
    expect(output.message.variant).toBe("max");
    expect(toastCalls).toContain("Ultrapower Mode Activated");
  });

  test("should trigger ultrapower on 'ultrapower' keyword", async () => {
    // #given - text contains 'ultrapower' keyword
    const toastCalls: string[] = [];
    const hook = createKeywordDetectorHook(
      createMockPluginInput({ toastCalls }),
    );
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrapower build this app" }],
    };

    // #when - message with 'ultrapower' is processed
    await hook["chat.message"]({ sessionID: "any-session" }, output);
  ...
  test("should NOT trigger ultrapower on partial matches like 'ulol'", async () => {
    // #given - text contains 'ulo' as part of another word
    const toastCalls: string[] = [];
    const hook = createKeywordDetectorHook(
      createMockPluginInput({ toastCalls }),
    );
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ulol something else" }],
    };

    // #when - message with partial 'ulo' match is processed
    await hook["chat.message"]({ sessionID: "any-session" }, output);

    // #then - ultrapower should NOT be triggered
    expect(toastCalls).not.toContain("Ultrapower Mode Activated");
  });

  test("should allow ultrapower keywords in non-main session", async () => {
    // #given - main session is set, different session submits ultrapower keyword
    const mainSessionID = "main-123";
    const subagentSessionID = "subagent-456";
    setMainSession(mainSessionID);
    ...
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrapower mode" }],
    };

    // #when - non-main session triggers ultrapower keyword
    await hook["chat.message"]({ sessionID: subagentSessionID }, output);

    // #then - ultrapower should still work (variant set to max)
    expect(output.message.variant).toBe("max");
    expect(toastCalls).toContain("Ultrapower Mode Activated");
  });
});

describe("keyword-detector agent-specific ultrapower messages", () => {
  let logCalls: Array<{ msg: string; data?: unknown }>;
  let logSpy: ReturnType<typeof spyOn>;
  ...
  test("should use planner-specific ultrapower message when agent is prometheus", async () => {
    // #given - collector and prometheus agent
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "ultrapower-prometheus-session";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrapower plan this feature" }],
    };

    // #when - ultrapower keyword detected with prometheus agent
    await hook["chat.message"]({ sessionID, agent: "prometheus" }, output);

    // #then - should use planner-specific message
    const pending = collector.getPending(sessionID);
    const ultrapowerEntry = pending.entries.find(
      (e) => e.id === "keyword-ultrapower",
    );
  ...
  test("should use executor-specific ultrapower message when agent is Sisyphus", async () => {
    // #given - collector and Sisyphus agent
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "ultrapower-sisyphus-session";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrapower implement this feature" }],
    };

    // #when - ultrapower keyword detected with Sisyphus agent
    await hook["chat.message"]({ sessionID, agent: "Sisyphus" }, output);

    // #then - should use executor-specific message with full workflow
    const pending = collector.getPending(sessionID);
    const ultrapowerEntry = pending.entries.find(
      (e) => e.id === "keyword-ultrapower",
    );
  ...
  test("should use executor-specific ultrapower message when agent is undefined", async () => {
    // #given - collector with no agent specified
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "ultrapower-no-agent-session";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ulo do something" }],
    };

    // #when - ultrapower keyword detected without agent
    await hook["chat.message"]({ sessionID }, output);

    // #then - should use executor-specific message (default behavior)
    const pending = collector.getPending(sessionID);
    const ultrapowerEntry = pending.entries.find(
      (e) => e.id === "keyword-ultrapower",
    );
  ...
  test("should have critical priority for ultrapower keywords", async () => {
    // #given - collector for ultrapower keyword
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "ultrapower-priority-session";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrapower implement" }],
    };

    // #when - ultrapower keyword detected
    await hook["chat.message"]({ sessionID }, output);
    ...
    expect(ultrapowerEntry).toBeDefined();
    expect(ultrapowerEntry!.priority).toBe("critical");
  });
});
