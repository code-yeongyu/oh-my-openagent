...
export function storeMemory(
  ...
  db.run(
    `INSERT INTO memories (id, content, embedding, agent_name, session_id, memory_type, importance, created_at, access_count, file_path, symbol_name, ast_pattern, before_content, after_content)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      content,
      ...
      options.symbolName ?? null,
      options.astPattern ?? null,
      options.beforeContent ?? null,
      options.afterContent ?? null,
    ],
  )
