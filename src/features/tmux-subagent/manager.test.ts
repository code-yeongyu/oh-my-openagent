...
describe('TmuxSessionManager', () => {
  ...
  describe('constructor', () => {
    test('enabled when config.enabled=true and isInsideTmux=true', async () => {
      ...
      // when
      const manager = new TmuxSessionManager(ctx, null, config, mockTmuxDeps)
    ...
    test('falls back to default port when serverUrl has port 0', async () => {
      ...
      try {
        ...
        // when
        manager = new TmuxSessionManager(ctx, null, config, mockTmuxDeps)
      } finally {
  ...
  describe('onSessionCreated', () => {
    ...
    test('skips deferred attach when the session is already pending through another spawn path', async () => {
      ...
      const { TmuxSessionManager } = await import('./manager')
      const manager = new TmuxSessionManager(createMockContext(), null, createTmuxConfig({ enabled: true }), mockTmuxDeps)
    ...
    test("skips pane creation when session exists but status is 'error'", async () => {
      ...
      const manager = new TmuxSessionManager(
        createMockContext({ sessionStatusResult }), null,
    ...
    test('expires failed readiness sessions after the TTL elapses', async () => {
      ...
      const { TmuxSessionManager } = await import('./manager')
      const manager = new TmuxSessionManager(
        createMockContext({ sessionStatusResult: { data: { ses_expired: { type: 'idle' } } } }), null,
        createTmuxConfig({ enabled: true }),
        mockTmuxDeps,
      )
    ...
    test('does not retry failed readiness sessions after polling marked the session closed', async () => {
      ...
      const { TmuxSessionManager } = await import('./manager')
      const manager = new TmuxSessionManager(
        createMockContext({ sessionStatusResult: { data: { ses_bounce: { type: 'idle' } } } }), null,
        createTmuxConfig({ enabled: true }),
        mockTmuxDeps,
      )
  ...
  describe('cleanup', () => {
    test('#given session isolation with two tracked panes #when polling closes both sessions #then it reassigns the anchor and cleans up the isolated container', async () => {
      ...
      const { TmuxSessionManager } = await import('./manager')
      const manager = new TmuxSessionManager(createMockContext(), null, createTmuxConfig({
        enabled: true,
        isolation: 'session',
      }), mockTmuxDeps)
    ...
    test('#given two manager instances #when both cleanup #then each kills its own isolated session name, not a shared one', async () => {
      ...
      mockKillTmuxSessionIfExists.mockClear()
      const { TmuxSessionManager } = await import('./manager')
      const managerA = new TmuxSessionManager(createMockContext(), null, createTmuxConfig({
        enabled: true,
        isolation: 'session',
      }), mockTmuxDeps)
      const managerB = new TmuxSessionManager(createMockContext(), null, createTmuxConfig({
        enabled: true,
        isolation: 'session',
      }), mockTmuxDeps)
