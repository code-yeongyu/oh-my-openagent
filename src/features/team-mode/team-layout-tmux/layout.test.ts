...
describe("team-layout-tmux", () => {
  ...
  test("creates focus and grid windows", async () => {
    ...
    //#then
    expect(spawnMock.mock.calls.flatMap((call) => (call as unknown as [Array<string>])[0])).toContain("new-session")
    expect(spawnMock.mock.calls.flatMap((call) => (call as unknown as [Array<string>])[0])).toContain("new-window")
    expect(spawnMock.mock.calls.flatMap((call) => (call as unknown as [Array<string>])[0])).toContain("split-window")
    expect(spawnMock.mock.calls.flatMap((call) => (call as unknown as [Array<string>])[0])).toContain("select-layout")
    expect(spawnMock.mock.calls.flatMap((call) => (call as unknown as [Array<string>])[0])).toContain("select-pane")
  })
  ...
  test("cleans up the tmux session", async () => {
    ...
    //#then
    expect(spawnMock.mock.calls.some((call) => ((call as unknown as [Array<string>])[0]).includes("kill-session"))).toBe(true)
  })
})
