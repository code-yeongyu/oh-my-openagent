...
const PATTERNS: Array<{ regex: RegExp; action: RecoveryAction; type: string }> = [
  ...
  { regex: /\bservice unavailable\b/i, action: "COOLING", type: "server_error" },
  {
    regex: /(?:model|deployment|endpoint|provider)\b[\s\S]{0,40}\bunavailable\b/i,
    action: "COOLING",
    type: "availability",
  },
  {
    regex: /(?:model|deployment|endpoint|provider)\b[\s\S]{0,40}\bnot found/i,
    action: "COOLING",
    type: "availability",
  },
  {
    regex: /(?:model|deployment|endpoint|provider)\b[\s\S]{0,40}\bdoes not exist/i,
    action: "COOLING",
    type: "availability",
  },
  {
    regex: /(?:model|deployment|endpoint|provider)\b[\s\S]{0,40}\bunsupported/i,
    action: "COOLING",
    type: "availability",
  },
