 ...
 * projects the Zod schema into the runtime OrchestratorConfig, applies
 * defaults for missing fields, and respects user overrides.
 */

import { describe, expect, it } from "bun:test"
import { MetaGovernorConfigSchema } from "../../config/schema/meta-governor"
import {
...
describe("loadOrchestratorConfig", () => {
  ...
  describe("#given enabled config", () => {
    const config = MetaGovernorConfigSchema.parse({ enabled: true })
  ...
  describe("#given custom memory config", () => {
    const config = MetaGovernorConfigSchema.parse({
      enabled: true,
      memory: {
...
describe("isMetaGovernorEnabled", () => {
  ...
  it("#given disabled config then returns false", () => {
    const config = MetaGovernorConfigSchema.parse({ enabled: false })
