...
const OhMyOpenCodePlugin: Plugin = async (ctx) => {
	...
	const enterpriseSecurity = isHookEnabled("enterprise-security")
		? createEnterpriseSecurityHook(ctx)
		: null;
	...
	return {
		...
		"tool.execute.before": async (input, output) => {
			...
			await enterpriseSecurity?.["tool.execute.before"]?.(input, output);
		...
		"tool.execute.after": async (input, output) => {
			...
			await enterpriseSecurity?.["tool.execute.after"]?.(input, output);
