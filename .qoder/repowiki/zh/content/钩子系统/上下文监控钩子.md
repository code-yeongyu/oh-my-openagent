...
- [src/hooks/start-work/index.test.ts](file://src/hooks/start-work/index.test.ts)
...
## 更新摘要
**所做更改**
- 增强了上下文监控钩子的类型安全性，新增了严格的接口定义和类型检查
- start-work钩子增加了智能执行模式选择功能，支持自动推荐执行模式
- 完善了系统指令类型的枚举定义，提升了类型安全性和兼容性
- 增强了上下文注入器的数据模型类型定义
...
- **新增** start-work 钩子的智能执行模式选择功能，支持自动推荐执行模式。
...
- [src/hooks/start-work/index.ts](file://src/hooks/start-work/index.ts#L1-L307)
...
## 核心组件
- 上下文窗口监控钩子：在工具执行后读取会话消息，计算当前输入令牌占比，超过阈值时向输出追加系统提醒与状态摘要。**已增强类型安全性**。
...
- **新增** start-work 钩子：智能执行模式选择，根据任务数量自动推荐执行模式（串行或并行），支持用户手动选择或自动决策。
...
- [src/hooks/start-work/index.ts](file://src/hooks/start-work/index.ts#L41-L77)
...
- **类型安全增强**：新增严格的接口定义，包括 `AssistantMessageInfo` 和 `MessageWrapper` 接口，确保类型安全。
...
- **类型安全增强**：完善了 `ContextEntry`、`RegisterContextOptions`、`PendingContext` 等接口定义，确保数据模型的一致性。
...
### start-work 钩子（智能执行模式选择）
- **新增功能**：智能执行模式选择，根据任务数量自动推荐执行模式（串行或并行）。
- 执行模式推荐逻辑：
  - 任务数 ≤ 5：推荐串行执行（逐个执行，适合依赖任务或 ≤5 个任务）
  - 任务数 > 5：推荐并行执行（并行波次执行，适合 >5 个独立任务）
  - 用户可选择"自动选择"，系统根据任务数自动决定
- 会话代理管理：触发 start-work 命令时自动清理之前的会话代理，允许模式转换。
- 计划管理：支持自动选择计划、显示进度状态、处理多个计划的情况。

```mermaid
flowchart TD
Start(["start-work 命令触发"]) --> CheckPlan{"是否有显式计划名?"}
CheckPlan -- 是 --> FindPlan["查找匹配计划"]
FindPlan --> PlanFound{"找到计划?"}
PlanFound -- 是 --> CheckComplete{"计划已完成?"}
CheckComplete -- 是 --> ShowComplete["显示已完成状态"]
CheckComplete -- 否 --> CreateState["创建 boulder 状态"]
CreateState --> RecommendMode["生成执行模式推荐"]
RecommendMode --> InjectPrompt["注入执行模式选择提示"]
ShowComplete --> End(["结束"])
CheckPlan -- 否 --> CheckExisting{"是否有现有状态?"}
CheckExisting -- 是 --> CheckProgress{"进度是否完成?"}
CheckProgress -- 否 --> ResumeSession["恢复现有会话"]
ResumeSession --> RecommendMode
CheckProgress -- 是 --> FindNewPlan["查找新计划"]
FindNewPlan --> SinglePlan{"只有一个未完成计划?"}
SinglePlan -- 是 --> AutoSelect["自动选择计划"]
AutoSelect --> RecommendMode
SinglePlan -- 否 --> ShowMultiple["显示多个计划供选择"]
ShowMultiple --> End
CheckExisting -- 否 --> NoPlan["无计划可用"]
NoPlan --> End
```

**图表来源**
- [src/hooks/start-work/index.ts](file://src/hooks/start-work/index.ts#L109-L304)

**章节来源**
- [src/hooks/start-work/index.ts](file://src/hooks/start-work/index.ts#L1-L307)
- [src/hooks/start-work/index.test.ts](file://src/hooks/start-work/index.test.ts#L1-L403)

## 依赖关系分析
- 上下文监控钩子依赖系统指令工具生成统一前缀，确保内部消息可被识别与过滤。
- 恢复钩子依赖错误解析器、存储层（读取/写入工具输出）、会话摘要与重试配置。
- 注入器依赖上下文收集器与会话状态（主会话 ID），在消息变换与聊天消息阶段注入。
- **新增** start-work 钩子依赖 boulder 状态管理、会话状态清理和计划发现功能。
...
- 上下文监控钩子
  ...
  - **类型安全增强**：严格的接口定义减少了运行时类型检查开销，提升了性能。
- Anthropic 上下文限制恢复钩子
...
- 上下文注入器
  ...
  - **类型安全增强**：完善的类型定义确保了编译时检查，减少了运行时错误。
- **新增** start-work 钩子
  - 计划查找和进度计算为 O(n) 复杂度，其中 n 为计划数量；通过缓存机制提升性能。
  - 执行模式推荐逻辑简单高效，主要基于任务数量比较。

## 故障排查指南
- 上下文监控未生效
  - 检查会话是否存在助手消息且 providerID 为 Anthropic；确认工具执行后钩子回调是否触发。
  - 确认会话删除事件是否正确清理提醒集合，避免重复提醒。
  - **类型安全检查**：确认 `AssistantMessageInfo` 接口定义是否正确，避免类型不匹配。
- 恢复钩子未触发或无效
...
- 注入器未注入
  ...
  - **类型安全检查**：确认 `ContextEntry` 接口的 `content` 字段是否为字符串类型。
- **新增** start-work 钩子问题
  - 检查计划文件格式是否正确，Markdown 复选框格式是否符合预期。
  - 确认 boulder 状态文件是否正确创建和更新。
  - 验证执行模式推荐逻辑是否按预期工作，检查任务数量计算准确性。
...
## 结论
Oh My OpenCode 的上下文监控与恢复体系通过"监控-解析-修剪-摘要-重试-注入"的闭环，有效应对 Anthropic 上下文窗口限制带来的会话中断风险；同时通过系统指令前缀与多策略注入，确保内部消息与外部输出清晰分离。**最新的类型安全增强**进一步提升了系统的稳定性和可靠性。**新增的智能执行模式选择功能**为 start-work 钩子提供了更智能化的用户体验，能够根据任务特征自动推荐最优的执行策略。合理配置阈值、修剪策略与恢复算法，可在保证任务完整性的同时提升稳定性与用户体验。
...
- 阈值
  - 使用率阈值：超过该比例时触发提醒（默认 70%）。
- 提醒内容
  - 包含系统指令前缀与当前使用/剩余百分比、令牌数摘要。
- **类型安全增强**
  - `AssistantMessageInfo` 接口：严格定义助手消息的令牌信息结构
  - `MessageWrapper` 接口：包装消息信息，支持可选的助手消息详情
...
- **类型安全增强**
  - `ParsedTokenLimitError` 接口：定义解析后的令牌限制错误结构
  - `AutoCompactState` 接口：管理会话状态的完整类型定义
...
- 会话状态
  - 待处理上下文、消费后清空，避免重复注入。
- **类型安全增强**
  - `ContextEntry` 接口：完整的上下文条目定义，包含元数据支持
  - `RegisterContextOptions` 接口：注册选项的类型安全定义
  - `PendingContext` 接口：待处理上下文的结构化定义
  - `OutputParts` 接口：输出部件的类型约束
...
- 计划管理
  ...
  - 处理多个计划的情况，提供用户选择界面
- 会话代理管理
  - 触发时自动清理之前的会话代理，允许模式转换
- **类型安全增强**
  - `StartWorkHookInput` 接口：严格定义输入参数结构
  - `StartWorkHookOutput` 接口：约束输出部件的类型定义
