...
describe("focused package registration audit", () => {
  ...
  test("#given CodeGraph MCP package registration #when root package wiring is audited #then workspace typecheck and dev dependency entries exist", async () => {
    // given
    const root = await readRootManifest()
    const manifest = await readManifest("packages/codegraph-mcp/package.json")

    // when
    const typecheckScript = root.scripts["typecheck:packages"] ?? ""

    // then
    expect(root.workspaces).toContain("packages/codegraph-mcp")
    expect(typecheckScript).toContain("tsgo --noEmit -p packages/codegraph-mcp/tsconfig.json")
    expect(root.devDependencies[manifest.name]).toBe("workspace:*")
