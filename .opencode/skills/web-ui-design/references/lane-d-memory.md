...
Lane D records the design record around OmO work. It owns `.omo/web-ui-design/state.md`, design debt, handoff notes, retrospective notes, and observational taste memory. It has no hooks and no independent automation path. It records and summarizes what happened so the next OpenAgent phase has reliable context.

## OmO Phase Owner

| Capability | Source boundary | OmO owner | Mapping |
|---|---|---|---|
| Track deferred design and accessibility findings | `design-debt-tracker` | `.omo/web-ui-design/state.md` plus final `review-work` context | Maintain a register with ID, date, source, severity, issue, affected users, suggested fix, status, and notes. |
...
| Maintain observational design memory | Authored concept preserved from excluded raw `design-memory` | `.omo/web-ui-design/state.md` and optional personal report context | Store observations descriptively. Do not feed them back as future project constraints. |
...
| Route designpowers concepts into OmO | Authored router semantics only; raw `using-designpowers` is excluded | web-ui-design router context | OmO router owns routing and mode language; no raw upstream router path or separate runtime is available. |

Vendored source agents are not primary in this lane. Lane D records outputs from `design-strategist`, `design-lead`, `motion-designer`, `content-writer`, `design-critic`, `accessibility-reviewer`, and `heuristic-evaluator` when those role references contributed context in earlier lanes.
...
Update `.omo/web-ui-design/state.md` with:
...
- handoff notes for the next OmO owner
...
- `.omo/web-ui-design/state.md` exists before a lane claims durable state.
...
- The evidence index points to real files produced by OmO planning, `start-work`, `visual-qa`, or `review-work`.
...
- Handoff text must be useful to the next OmO owner, not a narrative transcript.
